...
function TokoVoip.initialize(self)
	...
	TriggerEvent('ev-hud:client:UpdateVoiceProximity', self.mode)
	Citizen.CreateThread(function()
		TriggerEvent("ev-hud:client:UpdateVoiceProximity", 2)
		while (true) do
			Citizen.Wait(5);
			...
			if (IsDisabledControlJustPressed(0, self.keyProximity)) then -- Switch proximity modes (normal / whisper / shout)
				...
				if self.mode == 1 then
					TriggerEvent("ev-hud:client:UpdateVoiceProximity", 2)
				elseif self.mode == 2 then
					TriggerEvent("ev-hud:client:UpdateVoiceProximity", 1)
				else
					TriggerEvent("ev-hud:client:UpdateVoiceProximity", 3)
				end
			end
