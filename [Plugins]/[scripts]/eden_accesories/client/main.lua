...
AddEventHandler('eden_accesories:silencieux', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used < silencieux then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
				   ...
				   TriggerEvent('notification', 'You used a silencieux', 1)
		  		 	used = used + 1
	...
	used = used + 1
				...
				TriggerEvent('notification', 'You do not have a weapon in your hand or your weapon cannot support a silencieux', 1)		  		
			end
			else
	TriggerEvent('notification', 'You have to use all your silencieux', 1)
		end
        end
    end)
end)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used2 < flashlight then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("בדיוק הצטיידת במנורה. זה יהיה צריך להיות מצויד מחדש בכל פעם בעיר.")) 
					   used2 = used2 + 1
...
AddEventHandler('eden_accesories:grip', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used3 < grip then
			...
			if currentWeaponHash == GetHashKey("WEAPON_COMBATPDW") then
		  		 ...
		  		 TriggerEvent('notification', 'You used a grip', 1) 
		  				used3 = used3 + 1
	...
	used3 = used3 + 1
				...
				TriggerEvent('notification', 'You do not have a weapon in your hand or your weapon cannot support a grip', 1) 
			end
		else
			TriggerEvent('notification', 'Vous avez utiliser toutes vos poignées.', 1) 
		end
        end
    end)
end)
...
AddEventHandler('eden_accesories:yusuf', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used4 < yusuf then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 TriggerEvent('notification', 'You changed weapon color', 1) 
		  		 	used4 = used4 + 1
	...
	used4 = used4 + 1
				...
				TriggerEvent('notification', 'You do not have a weapon in your hand or your weapon cannot support change colors', 1) 		  		
			end
		else
