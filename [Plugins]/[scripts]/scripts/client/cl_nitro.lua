...
Citizen.CreateThread(function()
  while true do
    ...
    if GetPedInVehicleSeat(veh, -1) == ped then
      DrawAdvancedText(0.140, 0.90, 0.20, 0.0130, 0.4, "nos: " .. nitro, 255, 255, 255, 255, 6, 1)
    end
...
AddEventHandler('nitro:check', function()
  ...
  local veh = GetVehiclePedIsIn(ped, false)
  local hash = GetEntityModel(veh)
  if GetEntitySpeed(veh)>0.1 then TriggerEvent('notification', 'Vehicle must be stopped!', 2); return end
  if veh and IsThisModelACar(hash) then
    TriggerServerEvent('nitro:removeInventoryItem','nitro', 1)
  else
    TriggerEvent('notification', 'You need to be in a vehicle to install this!', 2)
  end
end)
...
AddEventHandler('nitro:activated', function()
  ...
  exports['taskbar']:taskBar(10100, 'Installing Nitrous')
  engineon = false
  Wait(3000)
  ...
  TriggerEvent('notification', 'Nitrous Installed', 1)
  engineon = true
  nitro = 100
end)
