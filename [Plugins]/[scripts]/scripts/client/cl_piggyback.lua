...
RegisterCommand("piggyback",function(source, args)
	if not piggyBackInProgress then
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			TriggerServerEvent('cmg2_animations:Start', closestPlayer, lib, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget)
		else 
			TriggerEvent('notification','No one nearby to piggyback!', 2)
...
RegisterNetEvent('cmg2_animations:Target')
AddEventHandler('cmg2_animations:Target', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag)
...
RegisterNetEvent('cmg2_animations:Me')
AddEventHandler('cmg2_animations:Me', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	RequestAnimDict(animationLib)
