...
function takeHostage()
	...
	if not canTakeHostage then 
		TriggerEvent('notification', "You cant take hostage with this weapon!", 2)
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			TriggerEvent('notification', "No one nearby to take as hostage!", 2)
		end 
	end
...
AddEventHandler('takehostage:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
...
RegisterNetEvent('takehostage:syncMe')
AddEventHandler('takehostage:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	ClearPedSecondaryTask(GetPlayerPed(-1))
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z+0.3,"Press G to release player")
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z+0.2,"Press H to kill player")
			...
			elseif IsDisabledControlJustPressed(0,74) then --kill 
				if hasAmmo() then
					...
					TriggerEvent('notification', 'You dont have ammo in your weapon.', 2)
				end
			end
		end
...
function hasAmmo()
	...
	for k,v in pairs(notNeedAmmo) do
		if GetHashKey(v) == weaponHash then
			return true
		end
	end

	if weaponHash and GetAmmoInPedWeapon(GetPlayerPed(-1), weaponHash) > 0 then
		return true
	end
