	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				TriggerServerEvent('logs:Died', GetPlayerName(PlayerId()) .. ' ' .. DeathReason .. '.', Weapon)
			else
				TriggerServerEvent('logs:Died', GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. '.', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
