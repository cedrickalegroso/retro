...
AddEventHandler('animation:Chair2', function()
	local ped = GetPlayerPed(-1)
		if recentlysat or exports["isPed"]:isPed("disabled") or IsPedInAnyVehicle(GetPlayerPed(-1), false) then
			return
		end
		recentlysat = true
	--	TaskStartScenarioAtPosition(ped, objinfo.scenario, objloc.x, objloc.y, objloc.z-objinfo.verticalOffset, GetEntityHeading(object)+180.0, 0, true, true)
		...
		local pos = GetOffsetFromEntityInWorldCoords(GetPlayerPed(-1), 0.0, -0.65, 0.0)

		local chair = CreateObject( GetHashKey('prop_chair_01a'), pos.x, pos.y, pos.z, true, false, false)
...
AddEventHandler('animation:Chair', function()
	...
	for k,v in pairs(objects) do
		...
		table.insert(list, {object = obj, distance = dist, name = v})
	end
	...
	if distance < Config.MaxDistance and DoesEntityExist(object) then
		...
		--if "prop_roller_car_01" == name then
		--	TriggerEvent("startCoaster",object)
		--end
		sit(object)

	else
		print("Sitting but no chair was detected, dood.")
	end
...
function sit(object)
	...
	local ped = GetPlayerPed(-1)
	lastPos = GetOffsetFromEntityInWorldCoords(GetPlayerPed(-1), 0.0, 0.0, -1.0)
