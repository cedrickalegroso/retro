...
RegisterCommand("carry",function(source, args)
	if not carryingBackInProgress then
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			TriggerServerEvent('CarryPeople:Start', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget)
		else
...
RegisterNetEvent('CarryPeople:Target')
AddEventHandler('CarryPeople:Target', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag)
...
RegisterNetEvent('CarryPeople:Me')
AddEventHandler('CarryPeople:Me', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	RequestAnimDict(animationLib)
