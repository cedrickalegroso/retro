...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened("emotes") then
            ...
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.1,"~g~Shift~s~ with F keys saves also. (F8 Doesnt Save)")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.0,"You can also type ~g~/e emotename~s~ in chat to perform them")
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        if IsControlJustReleased(0, 5) then
