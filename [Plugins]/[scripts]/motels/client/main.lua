...
RegisterCommand('raidroom', function(source, args)
 ...
 if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
  ...
  if not found then
   TriggerEvent('chatMessage', "This room isn't owned", 2)
  end
 else
  TriggerEvent('chatMessage', "You are not police", 2)
 end
end)

function OpenStash(owner, motel, room)
  TriggerEvent('ev-inventory:MotelBed', '' .. owner .. ' Motel Bed Storage')
end

function OpenInv(owner, motel, room)
  TriggerEvent('ev-inventory:MotelBed', '' .. owner .. ' Motel Storage')
end
 ...
 while true do
  ...
  if showmenushit then
   ...
   if stashDist < 1.2 then
    ...
    DrawText3D(myRoom.x-1.6, myRoom.y+1.2, myRoom.z+1.25, "[~b~E~w~] - access stash")
    if IsControlJustReleased(0, 38) then
      OpenInv(roomInfo.ident, roomInfo.motelid, roomInfo.roomno)
      --TriggerEvent("server-inventory-open", "1", "motel-stash"..myRoom.id.."-"..roomInfo.ident)
   ...
   if stashDist2 < 2.5 then
    DrawText3D(myRoom.x+1.6, myRoom.y+0.3, myRoom.z+0.25, "[~b~E~w~] - access bed stash")
    if IsControlJustReleased(0, 38) then
      OpenInv(roomInfo.ident, roomInfo.motelid, roomInfo.roomno)
      --TriggerEvent("server-inventory-open", "1", "motel-bed"..myRoom.id.."-"..roomInfo.ident)
     TriggerEvent('InteractSound_CL:PlayOnOne', 'motelstash', 0.6)
    end
   end
 end
  end
