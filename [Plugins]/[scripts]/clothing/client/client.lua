...
RegisterNetEvent('clothing:LoadYourClothes')
AddEventHandler('clothing:LoadYourClothes', function() LoadYourClothes() end)
RegisterCommand("removeprop", function() LoadYourClothes() end)

function LoadYourClothes()
    ESX.TriggerServerCallback('clothing:getPlayerSkin', function(skin)
	    if skin ~= nil then
            LoadPed(skin)
            Wait(1500)
            TriggerServerEvent("clothing:retrieve_tats")
	    end
    end)
end
...
RegisterNetEvent("clothing:toggleprop")
AddEventHandler("clothing:toggleprop", function(type)
    local all = {}
    all.name = type
    ToggleProps(all)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k,v in pairs(Config.YM) do
            for i=1, #v.YM, 1 do
                local dis = #(coords - v.YM[i])
                if dis < 1.2 then
                    DrawText3D(v.YM[i][1], v.YM[i][2], v.YM[i][3], 'Press ~r~[E]~s~ To Open The Menu')
                    local job = exports["isPed"]:isPed("myjob")
                    if dis < 3 and IsControlJustPressed(1, 38) then
                        if (job == "police" or 'ambulance') then
                        --if ESX.PlayerData.job.name == 'police' then
                        --exports['taskbar']:taskBar(2500, "Cloething")
                            TriggerEvent("clothing:openmenu")
                        else
                            TriggerEvent("notification",'Your Not A Officer/Ambulance',2)
                        end
                    end
                end
            end
...
RegisterNetEvent("clothing:inService")
AddEventHandler("clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent("clothing:openmenu")
AddEventHandler("clothing:openmenu", function()
    OpenMenu("clothesmenu")
end)


RegisterNetEvent("clothing:hasEnough")
AddEventHandler("clothing:hasEnough", function(menu)
...
RegisterNetEvent("clothing:defaultReset")
AddEventHandler("clothing:defaultReset", function()
...
RegisterNetEvent("clothing:settattoos")
AddEventHandler("clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("clothing:setpedfeatures")
AddEventHandler("clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
Citizen.CreateThread(function()
    ...
    for k,v in ipairs(tattoosShops) do
		...
		BeginTextCommandSetBlipName('STRING')
		AddTextComponentSubstringPlayerName('Tattoos Shop')
		EndTextCommandSetBlipName(blip)
	end
end)
