          ...
          if closestDist < self.InteractDist then
            if not safe then              
              ...
              if (not self.Looted or (self.Looted and not self.Looted[key])) and lootRemains then
                Utils:DrawText3D(val.Pos.x,val.Pos.y,val.Pos.z, "Press [~g~E~s~] to break the glass.")
                if Utils:GetKeyPressed("E") then
                  self:Interact(key,val, plyPed,false)
                end
              end
            elseif not self.SafeUsed then          
              Utils:DrawText3D(self.SafePos.x,self.SafePos.y,self.SafePos.z, "Press [~g~E~s~] to attempt to crack the safe.")
...
function MFV:DoNotifyPolice(...)
  if self.PlayerData.job.name == self.PoliceJobName then
    TriggerEvent('DoShortHudText', "Somebody is robbing Vangelico!, Go Fast!", 2)
    Wait(2000)
    TriggerEvent('DoShortHudText', "Somebody is robbing Vangelico!, Go Fast!", 2)  
    Wait(2000)
    TriggerEvent('notification', "Somebody is robbing Vangelico!, Go Fast!", 2)  
    Citizen.CreateThread(function(...)
      local blipA = AddBlipForRadius(-632.87689208984, -239.3971862793, 38.088756561279, 100.0)
      SetBlipHighDetail(blipA, true)
      SetBlipColour(blipA, 1)
      SetBlipAlpha (blipA, 128)

      local blipB = AddBlipForCoord(-632.87689208984, -239.3971862793, 38.088756561279)
