        ...
        for k, v in pairs(Config.Registers) do
            ...
            if dist <= 1 then
                ...
                if Config.Registers[k].robbed then
                    DrawText3Ds(Config.Registers[k].x, Config.Registers[k].y, Config.Registers[k].z, 'The cash safe is empty.')
                else
                    DrawText3Ds(Config.Registers[k].x, Config.Registers[k].y, Config.Registers[k].z, 'Use lockpick here to rob this store.')
                end
            end
        end
        if not inRange then 
                                ...
                                ESX.TriggerServerCallback('storerobbery:server:getOfficers', function(amount)
                                    ...
                                    if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                        ...
                                        if not copsCalled then
                                            ...
                                            TriggerServerEvent("storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                            copsCalled = true
                                        end
                                    else
                                        TriggerEvent('notification', "Not enough officers.", 2)
                                    end
                                end)
                            end
                        else
                        ...
                        Skillbar.Start({
                            ...
                            Skillbar.Start({
                                ...
                                if currentRegister ~= 0 then
                                    ...
                                    Progressbar("search_register", "Empty the cash safe.", lockpickTime, false, true, {
...
function Failed()
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerEvent('notification', "You tore yourself to the lockpick.")
    end
end
