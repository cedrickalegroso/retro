...
Padlock.Close = function() {
    ...
    $.post('http://storerobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('http://storerobbery/PadLockClose');
    if (data.error != null) {
        $.post('http://storerobbery/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("http://storerobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('http://storerobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('http://storerobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
...
function unlock() {
    gameOver = true;
    $.post('http://storerobbery/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
