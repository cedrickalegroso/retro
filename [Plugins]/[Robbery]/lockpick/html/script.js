...
Script.Open = function(data) {
    ...
    MainCounter=0;
    clearTimeout(killTimeout);
    killTimeout = setTimeout(Script.Failed, RotationTime);
}

Script.Failed = function() {
    if (active) $.post("http://lockpick/failed");
    active = false;
    Script.Close();
}

Script.Success = function() {
    if (active) $.post("http://lockpick/success");
    active = false;
    Script.Close();
}
...
Script.Close = function() {
    ...
    $.post('http://lockpick/closethermite');
}