...
RegisterNetEvent('HouseRobbery:attempt')
AddEventHandler('HouseRobbery:attempt', function(lockpicks)
 if isRobbing and DoesEntityExist(safe) and not safeOpen then
  ...
  if GetDistanceBetweenCoords(playerCoords, safepos.x, safepos.y, safepos.z, true) <= 3.0 then
   TriggerEvent('lockpick:client:openLockpick', function(success)
      if success then
         safeOpen = true
         TriggerServerEvent('HouseRobbery:giveMoney')
      else
         NotifyPolice()
      end
   end)
 ...
 if lockpicks > 0 and isNight() and not isRobbing then
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 4.5 then
      ...
      TriggerEvent('lockpick:client:openLockpick', function(success)
         ...
         if success then
            ...
            TriggerEvent('HouseRobbery:createHouse', id)
         else
            NotifyPolice()
            TriggerEvent('notification', 'You failed the lock',2)
 ...
 elseif not isNight() and not isRobbing then
  TriggerEvent('notification', 'Its too bright out to rob',2)
 end
end)

RegisterNetEvent('HouseRobbery:createHouse')
AddEventHandler('HouseRobbery:createHouse', function(id)
...
RegisterNetEvent('HouseRobbery:deleteHouse')
AddEventHandler('HouseRobbery:deleteHouse', function(id)
 local house = robbableHouses[id]
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Press E To Search ' .. myRobbableItems[i]["name"])
   ...
   local Skillbar = exports['skillbar']:GetSkillbarObject()
      ...
      Skillbar.Start({
         ...
         TriggerServerEvent('HouseRobbery:searchItem')
     end, function()
      ClearPedTasks(PlayerPedId())
     end)
...
RegisterNetEvent('lockpickAnimation')
...
function NotifyPolice()
   ...
   TriggerServerEvent('policealerts:server:AddPoliceAlert', {
     timeOut = 4000,
     alertTitle = "House Robbery In Progress.",
     coords = {
         x = coords.x,
         y = coords.y,
         z = coords.z,
