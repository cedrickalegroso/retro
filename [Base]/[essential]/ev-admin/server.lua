TriggerEvent("es:addGroup", "mod", "user","donator", function(group) end)

-- modify if you want, btw the _mod_ needs to be able to target the group and it will work
...
function loadExistingPlayers()
	TriggerEvent("es:getPlayers", function(curPlayers)
		for k,v in pairs(curPlayers)do
			TriggerClientEvent("ev-admin:setGroup", v.get('source'), v.get('group'))
		end
	end)
end
...
RegisterServerEvent('ev-admin:all')
AddEventHandler('ev-admin:all', function(type)
	local Source = source
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "superadmin", function(available)
			if available or user.getGroup() == "superadmin" then
				if type == "slay_all" then TriggerClientEvent('ev-admin:quick', -1, 'slay') end
				if type == "bring_all" then TriggerClientEvent('ev-admin:quick', -1, 'bring', Source) end
				if type == "slap_all" then TriggerClientEvent('ev-admin:quick', -1, 'slap') end
			else
				TriggerClientEvent('chat:addMessage', Source, {
...
RegisterServerEvent('ev-admin:quick')
AddEventHandler('ev-admin:quick', function(id, type)
	local Source = source
	TriggerEvent('es:getPlayerFromId', source, function(user)
			...
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						if type == "slay" then TriggerClientEvent('ev-admin:quick', id, type) end
						if type == "noclip" then TriggerClientEvent('ev-admin:quick', id, type) end
						if type == "freeze" then TriggerClientEvent('ev-admin:quick', id, type) end
						if type == "crash" then TriggerClientEvent('ev-admin:quick', id, type) end
						if type == "bring" then TriggerClientEvent('ev-admin:quick', id, type, Source) end
						if type == "goto" then TriggerClientEvent('ev-admin:quick', Source, type, id) end
						if type == "slap" then TriggerClientEvent('ev-admin:quick', id, type) end
						if type == "slay" then TriggerClientEvent('ev-admin:quick', id, type) end
						if type == "kick" then DropPlayer(id, 'Kicked by ev-admin GUI') end
						...
						if type == "ban" then
							...
							DropPlayer(id, GetConvar("ev-admin_banreason", "You were banned from this server"))
						end
					else
...
AddEventHandler('es:playerLoaded', function(Source, user)
	TriggerClientEvent('ev-admin:setGroup', Source, user.getGroup())
end)

RegisterServerEvent('ev-admin:set')
AddEventHandler('ev-admin:set', function(t, USER, GROUP)
	local Source = source
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "superadmin", function(available)
			if available then
					...
					TriggerEvent("es:getAllGroups", function(groups)
						if(groups[GROUP])then
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								TriggerClientEvent('ev-admin:setGroup', USER, GROUP)
...
-- Default commands
TriggerEvent('es:addCommand', 'superadmin', function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, {
...
TriggerEvent('es:addGroupCommand', 'noclip', "superadmin", function(source, args, user)
	TriggerClientEvent("ev-admin:noclip", source)
...
-- Freezing
local frozen = {}
TriggerEvent('es:addGroupCommand', 'freeze', "superadmin", function(source, args, user)
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			local player = tonumber(args[1])
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('ev-admin:freezePlayer', player, frozen[player])
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('ev-admin:teleportUser', target.get('source'), user.getCoords().x, user.getCoords().y, user.getCoords().z)
...
-- Slap
TriggerEvent('es:addGroupCommand', 'slap', "superadmin", function(source, args, user)
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			local player = tonumber(args[1])
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('ev-admin:slap', player)
		...
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('ev-admin:teleportUser', source, target.getCoords().x, target.getCoords().y, target.getCoords().z)
...
--	TriggerClientEvent('ev-admin:kill', source)
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('ev-admin:kill', player)
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('ev-admin:crash', player)
