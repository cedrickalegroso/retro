...
RegisterNetEvent('ev-admin:setGroup')
AddEventHandler('ev-admin:setGroup', function(g)
	group = g
end)
...
RegisterNUICallback('quick', function(data, cb)
	if data.type == "slay_all" or data.type == "bring_all" or data.type == "slap_all" then
		TriggerServerEvent('ev-admin:all', data.type)
	else
		TriggerServerEvent('ev-admin:quick', data.id, data.type)
	end
end)

RegisterNUICallback('set', function(data, cb)
	TriggerServerEvent('ev-admin:set', data.type, data.user, data.param)
end)

local noclip = false
RegisterNetEvent('ev-admin:quick')
AddEventHandler('ev-admin:quick', function(t, target)
...
RegisterNetEvent('ev-admin:freezePlayer')
AddEventHandler("ev-admin:freezePlayer", function(state)
	local player = PlayerId()
...
RegisterNetEvent('ev-admin:teleportUser')
AddEventHandler('ev-admin:teleportUser', function(x, y, z)
	SetEntityCoords(PlayerPedId(), x, y, z)
	states.frozenPos = {x = x, y = y, z = z}
end)

RegisterNetEvent('ev-admin:slap')
AddEventHandler('ev-admin:slap', function()
	local ped = PlayerPedId()

	ApplyForceToEntity(ped, 1, 9500.0, 3.0, 7100.0, 1.0, 0.0, 0.0, 1, false, true, false, false)
end)

RegisterNetEvent('ev-admin:kill')
AddEventHandler('ev-admin:kill', function()
	SetEntityHealth(PlayerPedId(), 0)
end)

RegisterNetEvent('ev-admin:heal')
AddEventHandler('ev-admin:heal', function()
	SetEntityHealth(PlayerPedId(), 200)
end)

RegisterNetEvent('ev-admin:crash')
AddEventHandler('ev-admin:crash', function()
	while true do
	end
end)

RegisterNetEvent("ev-admin:noclip")
AddEventHandler("ev-admin:noclip", function(t)
	local msg = "disabled"
	if(noclip == false)then
		noclip_pos = GetEntityCoords(PlayerPedId(), false)
	end
