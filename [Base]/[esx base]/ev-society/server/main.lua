...
AddEventHandler('esx_society:logs', function(data, src)
	...
	PerformHttpRequest(url, function(err, text, headers) end, 'POST', json.encode({username = "Society logs", content = "```css\n[" .. time .. "] " .. data .. "\n " .. msg .. "```"}), {['Content-Type'] = 'application/json'})
end)

function log(data, src)
	local id = source or src
    local time = os.date("%Y/%m/%d %X")
    local msg = 'Server name: '.. GetConvar('sv_hostname', 'Invaild Server Name') .. '\n Player IP: '.. GetPlayerEndpoint(id) .. '\n Player Steam: '.. GetPlayerIdentifier(id)
	PerformHttpRequest(url, function(err, text, headers) end, 'POST', json.encode({username = "Society logs", content = "```css\n[" .. time .. "] " .. data .. "\n " .. msg .. "```"}), {['Content-Type'] = 'application/json'})
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'lieutenant'
	...
	if isBoss then
		if salary <= Config.MaxSalary then
			MySQL.Async.execute('UPDATE job_grades SET salary = @salary WHERE job_name = @job_name AND grade = @grade', {
				...
				log(GetPlayerName(source) .. '(' .. source .. ') set job salary (' .. job .. ' | ' .. grade .. ') to $' .. salary)
