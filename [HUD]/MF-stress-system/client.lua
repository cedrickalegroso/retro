...
function MFS:Update(...)
  local lastHealth = GetEntityHealth(GetPlayerPed(-1))
    ...
    TriggerEvent('esx_status:getStatus','stress',function(status)
      ...
      if status.val >= (self.GetSickAt * 10000) then
        ...
        if not self.IsSick then
          self.IsSick = true
          ESX.ShowNotification("You have become sick from too much stress. Try some relieving activities.")
        else
          ...
          if status.val >= (self.ExtremelySickAt * 10000) then
            ...
            if not self.ExtremelySick then
              ...
              ESX.ShowNotification("You have become very sick from too much stress. Try some relieving activities.")
            ...
            if (GetGameTimer() - lastVomit) > self.VomitTimer * 1000 then    
              if not IsPedShooting(plyPed) and not IsPedInMeleeCombat(plyPed) then
                ...
                if hp-self.VomitHealthLoss > 100 then SetEntityHealth(GetPlayerPed(-1), hp - self.VomitHealthLoss); end
              end
            end
          elseif self.ExtremelySick then 
            self.ExtremelySick = false
            ESX.ShowNotification("You feel a little better. Keep up the calming activities.")
          end
        end
      elseif status.val <= (self.GetSickAt * 10000) then
        if self.IsSick then
          self.IsSick = false
          ESX.ShowNotification("You feel more relaxed.")
