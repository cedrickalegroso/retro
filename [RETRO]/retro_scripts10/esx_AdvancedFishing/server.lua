...
AddEventHandler("fishing:lowmoney", function(model)
	...
	local myModel = model
	local xPlayer = ESX.GetPlayerFromId(_source)
	for k, v in ipairs(Server_ConfigFishing.RentalBoats) do
...
AddEventHandler('fishing:startSelling', function(yaItem)
	...
	if item == "fish" then
		local FishQuantity = xPlayer.getInventoryItem('fish').count
			...
			local payment = Server_ConfigFishing.FishPrice.a
			payment = math.random(Server_ConfigFishing.FishPrice.a, Server_ConfigFishing.FishPrice.b) 
	...
	if item == "turtle" then
		...
		if FishQuantity <= 0 then
			...
			xPlayer.removeInventoryItem('turtle', 1)
			local payment = Server_ConfigFishing.TurtlePrice.a
			payment = math.random(Server_ConfigFishing.TurtlePrice.a, Server_ConfigFishing.TurtlePrice.b) 
	...
	if item == "shark" then
		...
		if FishQuantity <= 0 then
			...
			local payment = Server_ConfigFishing.SharkPrice.a
			payment = math.random(Server_ConfigFishing.SharkPrice.a, Server_ConfigFishing.SharkPrice.b)
			xPlayer.addAccountMoney('black_money', payment)
		end
	end
end)