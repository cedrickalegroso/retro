...
for _, v in ipairs(ConfigFishing.Sell) do
...
for _, info in pairs(ConfigFishing.MarkerZones) do
...
Citizen.CreateThread(function()
    while true do
		...
		-- Rental Markers
		for k, v in pairs(ConfigFishing.MarkerZones) do
				...
				if #(pedCoords - v.Return) < 5.0 then
					if DoesEntityExist(veh) and GetVehiclePedIsIn(ped) == veh then
						...
						if IsControlJustReleased(0, Keys['E']) then
							for _, q in pairs(ConfigFishing.RentalBoats) do
		...
		-- Sell markers
		for k, v in ipairs(ConfigFishing.Sell) do
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		...
		if pos.y >= 0 or pos.y <= 1 or pos.x <= 1 or pos.x >= 0 then
...
function OpenBoatsMenu(xyz, h)
	...
	for k, v in ipairs(ConfigFishing.RentalBoats) do
