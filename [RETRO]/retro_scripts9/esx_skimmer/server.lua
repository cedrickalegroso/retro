...
ESX.RegisterUsableItem('scandev', function(source)
	...
	if xPlayer.job.name ~= 'police' then
		if chance > 7 then
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Be careful RCPD might know this scanner device.'})
...
ESX.RegisterUsableItem('skimdev', function(source)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name ~= 'police' then
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Be careful RCPD might know this skimmer device.'})
...
AddEventHandler('esx_skimmer:applySkim', function()
	...
	repeat
		...
		local callthepolice =  math.random(0, 1)
		 


		if callthepolice == 0 then
        --ddooo
		elseif callthepolice == 1 then 
		TriggerClientEvent('retro_script:callcops', soure)
		end
