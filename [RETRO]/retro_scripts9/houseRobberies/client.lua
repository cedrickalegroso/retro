...
local robbableItems = {
 [1] = {x = 1.90339700, y = -3.80026800, z = 1.29917900, name = "refrigerador", isSearched = false},
 [2] = {x = -3.50363200, y = -6.55289400, z = 1.30625800, name = "cajones", isSearched = false},
 [3] = {x = -3.50712600, y = -4.13621600, z = 1.29625800, name = "mesa", isSearched = false},
 [4] = {x = 8.47819500, y = -2.50979300, z = 1.19712300, name = "cajon", isSearched = false},
 [5] = {x = 9.75982700, y = -1.35874100, z = 1.29625800, name = "baul", isSearched = false},
 [6] = {x = 8.46626300, y = 4.53223600, z = 1.19425800, name = "guardarropa", isSearched = false},
 [7] = {x = 5.84416200, y = 2.57377400, z = 1.22089100, name = "mesa", isSearched = false},
 [8] = {x = 3.04164100, y = 0.31671810, z = 3.58363900, name = "joyero", isSearched = false},
 [9] = {x = 6.86376900, y = 1.20651200, z = 1.36589100, name = "debajo de la cama", isSearched = false},
 [10] = {x = 2.03442400, y = -5.61585100, z = 3.30395600, name = "armarios", isSearched = false},
 [11] = {x = -5.53120400, y = 0.76299670, z = 1.77236000, name = "gabinete", isSearched = false},
 [12] = {x = -1.24716200, y = 1.07820500, z = 1.69089300, name = "mesa de noche", isSearched = false},
 ...
 if lockpicks > 0 and isNight() and not isRobbing then
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
	...
	exports['progressBars']:startUI(12000, "Forcing Entrance")
	Wait(12000)
--    exports["t0sic_loadingbar"]:StartDelayedFunction("Lockpicking Property", 12000, function()
     ...
     if math.random(1, 10) == 1 then
      TriggerServerEvent('MF_Trackables:Notify',"Alarma activada en una casa", GetEntityCoords(GetPlayerPed(-1)),'police')
     end
...
--  TriggerEvent('notification', 'No lockpick', 2)
  exports['mythic_notify']:DoHudText('error', 'No Lockpick')
...
--  TriggerEvent('notification', 'It\'s too bright out', 2)
  exports['mythic_notify']:DoHudText('error', 'Its too bright out to rob')
 else
...
AddEventHandler('houseRobberies:createHouse', function(id)
 ...
 while isRobbing do
  ...
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
   showMessage('Press ~g~[E]~w~ to leave the house.')
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Press ~g~[H]~s~ to look in ' .. myRobbableItems[i]["name"])
	...
	exports['progressBars']:startUI(12000, "Looking")
	Wait(12000)
	TriggerEvent('esx_status:add', 'stress', 25000)
   ...
   if disturbance > 85 then
    if not calledin then
     ...
     if fuckup == 2 and GetEntitySpeed(GetPlayerPed(-1)) > 0.8 then
      ...
      if not isAgro then
       agroNPC()
       TriggerServerEvent('MF_Trackables:Notify',"Alarma de robo a casa", GetEntityCoords(GetPlayerPed(-1)),'police')
      end
     end
    end
   end
...
function agroNPC()
 ...
 repeat
  ...
  if distance < 35.0 and ped ~= GetPlayerPed(-1) and not IsPedAPlayer(ped) then
   ...
   GiveWeaponToPed(ped, GetHashKey('WEAPON_PISTOL'), 150, 0, 1)
