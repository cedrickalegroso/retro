...
RegisterNetEvent("nef_phone:open")
AddEventHandler("nef_phone:open", function()
  TooglePhone()
end)
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
      exports['screenshot-basic']:requestScreenshotUpload('http://51.79.147.30:3555/upload/', 'files[]',  function(data)
       ...
       -- print(resp.files[1].url)
        ...
        cb(json.encode({ url = resp }))   

        --local ey = json.encode({ url = data })
