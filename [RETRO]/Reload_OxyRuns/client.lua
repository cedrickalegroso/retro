...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			TriggerEvent("notification", "The delivery was on point, your pager will be updated with the next drop off")
		else
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			TriggerEvent("notification","You are close to the drop off")
		end
		toolong = toolong - 1
		if toolong < 0 then
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true
	TriggerEvent("notification","A car has been left outside for you. Your pager will be updated with locations soon")
	CreateOxyVehicle()
end)