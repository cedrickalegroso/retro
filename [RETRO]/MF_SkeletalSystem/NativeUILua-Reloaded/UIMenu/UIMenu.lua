...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName, Heading, R, G, B, A)
    ...
    if TxtName ~= nil then
        TxtName = tostring(TxtName) or "gradient_bdg"
    else
        TxtName = "gradient_bdg"
    end
    if Heading ~= nil then
    ...
    local _UIMenu = {
        ...
        Position = { X = X, Y = Y },
        DrawOffset = { X = 0, Y = 0 },
        Pagination = { Min = 0, Max = 13, Total = 12 },
        PageCounter = {
            isCustom = false,
            PreText = "",
        },
...
function UIMenu:GoLeft()
    if not self.Items or not self:GetCurrentSelection() then return; end
    local type, subtype = self.Items[self:CurrentSelection()]()
