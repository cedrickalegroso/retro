...
function MFS:SetupBones()
  ESX.TriggerServerCallback('MF_SkeletalSystem:GetPlayerDamage', function(data)
    self.BoneCats = {}
    self.Bones = {}      
  ...
  while self.dS and self.cS do
    ...
    if (GetGameTimer() - timer) > self.SaveAt * 60 * 1000 then 
      ...
      if self.MarkedForSave then 
        TriggerServerEvent('MF_SkeletalSystem:SavePlayers', self.BoneCats)
...
RegisterNetEvent('MF_SkeletalSystem:DoUpdate')
AddEventHandler('MF_SkeletalSystem:DoUpdate', function(...) TriggerServerEvent('MF_SkeletalSystem:SavePlayers',MFS.BoneCats); end)
...
function MFS:DrawUI()
  if self.MenuOpen then
    ...
    local hp = math.max(GetEntityHealth(GetPlayerPed(-1)) - 100, 0)
    local str = ""
    ...
    templateB.text = "~t~ / " .. (GetEntityMaxHealth(GetPlayerPed(-1)) - 100)
    templateB.font = 1
    templateB.x = 0.415
    templateB.y = 0.755 
...
function MFS:Awake(...)
    while not ESX do Citizen.Wait(0); end
    while not ESX.IsPlayerLoaded() do Citizen.Wait(0); end
    ESX.TriggerServerCallback('MF_SkeletalSystem:GetStartData', function(retVal) self.dS = true; self.cS = retVal; self:Start(); end)
end
...
function MFS:CheckOther()
  ...
  Citizen.CreateThread(function(...) 
    ...
    ESX.TriggerServerCallback('MF_SkeletalSystem:GetOtherData', function(data) 
      self.ViewBoneCats = {}
      self.ViewBones = {}      
      for bType,v in pairs(self.PedBones) do
        if data then 
...
function MFS:UseItemOther(...)
  ...
  ESX.TriggerServerCallback('MF_SkeletalSystem:GetMedicItems', function(items) 
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Medic_Items", { title = "Medic Items", align = 'right', elements = elements }, 
      function(data,menu)
        ...
        TriggerServerEvent('MF_SkeletalSystem:UseItemOnOther', GetPlayerServerId(closest), data.current.name)       
      end,    
      function(data,menu)
        menu.close()
        ESX.UI.Menu.CloseAll()
...
RegisterNetEvent('MF_SkeletalSystem:UseItemMedic')
AddEventHandler('MF_SkeletalSystem:UseItemMedic', function(...) MFS:UseItemMedic(...); end)

RegisterNetEvent('MF_SkeletalSystem:HealBones')
AddEventHandler('MF_SkeletalSystem:HealBones', function(category) MFS:HealBones(category); end)

RegisterNetEvent('MF_SkeletalSystem:PauseDamage')
AddEventHandler('MF_SkeletalSystem:PauseDamage', function(category) MFS.PauseDamage = true; end)

RegisterNetEvent('MF_SkeletalSystem:EnableDamage')
AddEventHandler('MF_SkeletalSystem:EnableDamage', function(category) MFS.PauseDamage = false; end)
