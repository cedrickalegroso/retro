...
function ListOwnedPoliceMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedSCFCars', function(ownedSCFCars)
		if #ownedSCFCars == 0 then
			ESX.ShowNotification(_U('garage_no_police'))
		else
			for _,v in pairs(ownedSCFCars) do
				local hashVehicule = v.vehicle.model
				local aheadVehName = GetDisplayNameFromVehicleModel(hashVehicule)
				local vehicleName = GetLabelText(aheadVehName)
				local plate = v.plate
		...
		ESX.TriggerServerCallback('esx_advancedgarage:getOwnedSCFAircrafts', function(ownedSCFAircrafts)
			if #ownedAircrafts == 0 then
				ESX.ShowNotification(_U('garage_no_police_aircraft'))
			else
				for _,v in pairs(ownedSCFAircrafts) do
					local hashVehicule = v.vehicle.model
					local aheadVehName = GetDisplayNameFromVehicleModel(hashVehicule)
					local vehicleName = GetLabelText(aheadVehName)
					local plate = v.plate
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
			if valid then
				if engineHealth < 990 then
					if Config.UseDamageMult then
						local apprasial = math.floor((1000 - engineHealth)/1000*Config.SCFPoundPrice*Config.DamageMult)
						RepairVehicle(apprasial, vehicle, vehicleProps)
					else
						local apprasial = math.floor((1000 - engineHealth)/1000*Config.SCFPoundPrice)
						RepairVehicle(apprasial, vehicle, vehicleProps)
					end
				else
					StoreVehicle(vehicle, vehicleProps)
...
function ReturnOwnedSCFMenu()
	if WasinJPound then
		ESX.ShowNotification(_U('must_wait', Config.JPoundWait))
	else
		ESX.TriggerServerCallback('esx_advancedgarage:getOutOwnedSCFCars', function(ownedSCFCars)
			local elements = {}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_police', {
				title = _U('pound_police', ESX.Math.GroupDigits(Config.SCFPoundPrice)),
				align = Config.MenuAlign,
				elements = elements
			}, function(data, menu)
				ESX.TriggerServerCallback('esx_advancedgarage:checkMoneySCF', function(hasEnoughMoney)
					if hasEnoughMoney then
						if data.current.value == nil then
						else
							SpawnVehicle(data.current.value, data.current.value.plate)
							TriggerServerEvent('esx_advancedgarage:paySCF')
						end
					else
						ESX.ShowNotification(_U('not_enough_money'))
					end
...
AddEventHandler('esx_advancedgarage:hasEnteredMarker', function(zone)
	...
	elseif zone == 'police_pound_point' then
		...
		CurrentActionMsg = _U('press_to_impound')
		CurrentActionData = {}
	elseif zone == 'scf_garage_point' then
		CurrentAction = 'scf_garage_point'
		CurrentActionMsg = _U('press_to_enter')
		CurrentActionData = {}
	elseif zone == 'scf_store_point' then
		CurrentAction = 'scf_store_point'
		CurrentActionMsg = _U('press_to_delete')
		CurrentActionData = {}
	elseif zone == 'scf_pound_point' then
		CurrentAction = 'scf_pound_point'
		CurrentActionMsg = _U('press_to_impound')
		CurrentActionData = {}
