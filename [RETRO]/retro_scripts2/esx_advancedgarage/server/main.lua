...
-- Start of SCF Code
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedSCFCars', function(source, cb)
	local ownedSCFCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.ShowVehicleLocation then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
			...
			for _,v in pairs(data) do
				local vehicle = json.decode(v.vehicle)
				table.insert(ownedSCFCars, {vehicle = vehicle, stored = v.stored, plate = v.plate})
			end
			cb(ownedSCFCars)
		end)
	else
...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedSCFAircrafts', function(source, cb)
	local ownedSCFAircrafts = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.UsingAdvancedVehicleShop then
		if Config.ShowVehicleLocation then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedSCFAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'aircraft'})
				end
				cb(ownedSCFAircrafts)
			end)
		else
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', { -- job = NULL
				['@owner'] = xPlayer.identifier,
		...
		if Config.ShowVehicleLocation then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedSCFAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'helicopter'})
				end
				cb(ownedSCFAircrafts)
			end)
		else
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedSCFCars', function(source, cb)
	local ownedSCFCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND job = @job AND `stored` = @stored', {
		...
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
			table.insert(ownedSCFCars, vehicle)
		end
		cb(ownedSCFCars)
	end)
end)

ESX.RegisterServerCallback('esx_advancedgarage:checkMoneySCF', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getMoney() >= Config.SCFPoundPrice then
		cb(true)
	else
		cb(false)
	end
...
RegisterServerEvent('esx_advancedgarage:paySCF')
AddEventHandler('esx_advancedgarage:paySCF', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeMoney(Config.SCFPoundPrice)
	TriggerClientEvent('esx:showNotification', source, _U('you_paid') .. Config.SCFPoundPrice)

	if Config.GiveSocietyMoney then
		TriggerEvent('esx_addonaccount:getSharedAccount', 'society_mechanic', function(account)
			account.addMoney(Config.SCFPoundPrice)
		end)
	end
end)
-- End of Police Code
