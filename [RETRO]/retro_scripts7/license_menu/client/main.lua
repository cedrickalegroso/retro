...
RegisterNetEvent("nef_id:open")
AddEventHandler("nef_id:open", function()
	print('reached')
	OpenUsermenuMenu()
end)
...
function OpenUsermenuMenu()
	...
	for i=1, #ConfigID.Usermenu, 1 do
		table.insert(elements, {label = ConfigID.Usermenu[i].label, value = ConfigID.Usermenu[i].name})
	end
...
function OpenUsermenuSubMenu(menu)
	...
	for i=1, #ConfigID.Usermenu, 1 do
		if ConfigID.Usermenu[i].name == menu then
			title = ConfigID.Usermenu[i].label

			for j=1, #ConfigID.Usermenu[i].items, 1 do
				table.insert(elements, {
					label = ConfigID.Usermenu[i].items[j].label,
					type  = ConfigID.Usermenu[i].items[j].type
				})
			end
