...
AddEventHandler('loaf_housing:enterHouse', function(id)
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        ...
        if house.houseId == id then
            for k, v in pairs(ConfigLOAF.HouseSpawns) do
                if not v['taken'] then
...
AddEventHandler('loaf_housing:buy_furniture', function(category, id)
    ...
    if ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id] then
        if xPlayer.getAccount('bank').money >= ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][3] then
            xPlayer.removeAccountMoney('bank', ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][3])
            hadMoney = true
        else
            if xPlayer.getMoney() >= ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][3] then
                xPlayer.removeMoney(ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][3])
                hadMoney = true
            else
    ...
    if hadMoney then
        MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
            local furniture = json.decode(result[1]['bought_furniture'])
            if furniture[ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][2]] then 
                furniture[ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][2]]['amount'] = furniture[ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][2]]['amount'] + 1
            else
                furniture[ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][2]] = {['amount'] = 1, ['name'] = ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][1]}
            end
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, (Strings['Bought_Furniture']):format(ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][1], ConfigLOAF.Furniture[ConfigLOAF.Furniture['Categories'][category][1]][id][3]))
        end)
    end
end)
...
AddEventHandler('loaf_housing:deleteInstance', function()
    ...
    if instances[src] then
        ConfigLOAF.HouseSpawns[instances[src]['housespawn']]['taken'] = false
        ...
        if house.houseId == 0 then
            MySQL.Async.fetchAll("SELECT * FROM bought_houses WHERE houseid=@houseid", {['@houseid'] = id}, function(result)
                ...
                if not result[1] then
                    if xPlayer.getAccount('bank').money >= ConfigLOAF.Houses[id]['price'] then
                        xPlayer.removeAccountMoney('bank', ConfigLOAF.Houses[id]['price'])
                        ...
                        if xPlayer.getMoney() >= ConfigLOAF.Houses[id]['price'] then
                            xPlayer.removeMoney(ConfigLOAF.Houses[id]['price'])
                            MySQL.Sync.execute("INSERT INTO bought_houses (houseid) VALUES (@houseid)", {['houseid'] = id})
                            MySQL.Async.execute("UPDATE users SET house=@house WHERE identifier=@identifier", {['@identifier'] = xPlayer.identifier, ['@house'] = newHouse}) 
                        else
                            TriggerClientEvent('esx:showNotification', xPlayer.source, 'You dont have enough money')
                        end
                    end
                end
            end)
...
AddEventHandler('loaf_housing:sellHouse', function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        local house = json.decode(result[1].house)
        if ConfigLOAF.Houses[house.houseId]['price'] then
            xPlayer.addMoney(ConfigLOAF.Houses[house.houseId]['price'] * (ConfigLOAF.SellPercentage/100))
            TriggerClientEvent('esx:showNotification', xPlayer.source, ('House Sold'):format(math.floor(ConfigLOAF.Houses[house.houseId]['price'] * (ConfigLOAF.SellPercentage/100))))
...
ESX.RegisterServerCallback('loaf_housing:hostOnline', function(source, cb, host)
    ...
    if instances[host] then
        ...
        if not online then
            ConfigLOAF.HouseSpawns[instances[host]['housespawn']]['taken'] = false
            instances[host] = {}
        end
    end
    cb(online)
            ...
            if type == 'item' then
                ...
                TriggerEvent('esx_addoninventory:getInventory', 'housing', identifier, function(inventory)
                    if inventory.getItem(item)['count'] >= amount then
                        TriggerClientEvent('esx:showNotification', src, ('You withdrew'):format(amount, inventory.getItem(item)['label']))
        ...
        if instances[houses[owner]] then
            ...
            if type == 'item' then
                ...
                if xPlayer.getInventoryItem(item)['count'] >= amount then
                    TriggerEvent('esx_addoninventory:getInventory', 'housing', identifier, function(inventory)
                        ...
                        TriggerClientEvent('esx:showNotification', src, ('You stored'):format(amount, inventory.getItem(item)['label']))
                    end)
                else
                    TriggerClientEvent('esx:showNotification', src, 'Not enough')
                end
            ...
            elseif type == 'weapon' then
                ...
                if hasweapon then
                    ...
                    TriggerClientEvent('esx:showNotification', src, 'No weapons')
                end
            end
        end
