...
Citizen.CreateThread(function()
    ...
    if ConfigLOAF.IKEABlip['Enabled'] then
        local blip = AddBlipForCoord(ConfigLOAF.Furnituring['enter'])
        SetBlipSprite(blip, ConfigLOAF.IKEABlip['Sprite'])
        SetBlipColour(blip, ConfigLOAF.IKEABlip['Colour'])
        SetBlipAsShortRange(blip, true)
        SetBlipScale(blip, ConfigLOAF.IKEABlip['Scale'])
    ...
    while true do
        Wait(500)
        for k, v in pairs(ConfigLOAF.Houses) do
            if Vdist2(GetEntityCoords(PlayerPedId()), v['door']) <= 2.5 then
                ...
                while Vdist2(GetEntityCoords(PlayerPedId()), v['door']) <= 2.5 do
                    if OwnedHouse.houseId == k then
                        text = ('Press ~INPUT_CONTEXT~ to %s'):format('manage your house')
                    else
                        if not AvailableHouses[k] then
                            if OwnedHouse.houseId ~= 0 then
                                text = 'You can only own one house. Sell your house to buy this one.'
                            else
                               
                                text = ('Press ~INPUT_CONTEXT~ to %s'):format('Buy house #'..k..' for '..v['price']..'?')
                            end
                        else
                            text = ('Press ~INPUT_CONTEXT~ to %s'):format('knock on the door')
                        end
                    end
                    ...
                    if IsControlJustReleased(0, 38) then
                        if OwnedHouse.houseId == k then
                            ...
                            elements = {
                                ...
                                {label = ('Sell for $%s'):format(math.floor(ConfigLOAF.Houses[OwnedHouse.houseId]['price']*(ConfigLOAF.SellPercentage/100))), value = 'sell'},
                            }
                            if ConfigLOAF.EnableGarage then
                            ...
                            ESX.UI.Menu.Open(
                                ...
                                function(data, menu)
                                    ...
                                    elseif data.current.value == 'garage' then
                                        local coords = ConfigLOAF.Houses[OwnedHouse.houseId]['door']
                                        local found, coords, heading = GetClosestVehicleNodeWithHeading(coords.x, coords.y, coords.z, 3.0, 100.0, 2.5)
                                        if found then
                                            ESX.UI.Menu.CloseAll()
                                            ...
                                            ESX.ShowNotification('Couldn\'t find a spawnpoint for your house.')
                                        end
                                    elseif data.current.value == 'sell' then
                                        ESX.UI.Menu.Open(
                                            'default', GetCurrentResourceName(), 'sell',
                                        {
                                            title = ('Sell house for $%s?'):format(math.floor(ConfigLOAF.Houses[OwnedHouse.houseId]['price']*(ConfigLOAF.SellPercentage/100))),
                                            align = 'top-left',
                                            elements = {
                                                {label ='yes', value = 'yes'},
                                                {label = 'no', value = 'no'}
                            ...
                            if not AvailableHouses[k] and OwnedHouse.houseId == 0 then
                                ...
                                {
                                    title = ('Buy house #%s for $%s?'):format(k, v['price']),
                                    align = 'top-left',
                                    elements = {
                                        {label ='yes', value = 'yes'},
                                        {label = 'no', value = 'no'}
                                ...
                                if AvailableHouses[k] then
                                    ...
                                    while Vdist2(GetEntityCoords(PlayerPedId()), v['door']) <= 4.5 do Wait(0) HelpText('Waiting for the house owner to let you in.', v['door']) end
                ...
                if OwnedHouse.houseId == k then
                    ...
                    local found, coords, heading = GetClosestVehicleNodeWithHeading(coords.x, coords.y, coords.z, 3.0, 100.0, 2.5)
                    if found then
                        while #(GetEntityCoords(PlayerPedId()) - coords) <= 45.0 and IsPedInAnyVehicle(PlayerPedId(), false) and ConfigLOAF.EnableGarage do
                            Wait(0)
                            ...
                            if #(GetEntityCoords(PlayerPedId()) - coords) <= 5.0 and IsPedInAnyVehicle(PlayerPedId(), false) and ConfigLOAF.EnableGarage then
                                HelpText('Press ~INPUT_CONTEXT~ to store the vehicle', coords)
                                if IsControlJustReleased(0, 38) then
                                    ...
                                    ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
                                        if valid then
                                            if engineHealth < 990 then
                                                if ConfigLOAF.DamageMult then
                                                    local apprasial = math.floor((1000 - engineHealth)/1000*ConfigLOAF.PoundP*ConfigLOAF.MultAmount)
                                                    RepairVehicle(apprasial, vehicle, vehicleProps)
                                                else
                                                    local apprasial = math.floor((1000 - engineHealth)/1000*ConfigLOAF.PoundP)
                                                    RepairVehicle(apprasial, vehicle, vehicleProps)
                                                end
                                            else
...
Citizen.CreateThread(function()
    ...
    while true do
        Wait(0)
        local dist = Vdist2(GetEntityCoords(PlayerPedId()), ConfigLOAF.Furnituring['enter']) 
        if dist <= 50.0 then
            DrawMarker(27, ConfigLOAF.Furnituring['enter'], vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 150, false, false, 2, false, false, false)
            if dist <= 1.5 then
                HelpText(('Press ~INPUT_CONTEXT~ to %s'):format('enter IKEA'), ConfigLOAF.Furnituring['enter'])
                if IsControlJustReleased(0, 38) then
                    FreezeEntityPosition(PlayerPedId(), true)
                    local currentcategory = 1
                    local category = ConfigLOAF.Furniture['Categories'][currentcategory]

                    local current = 1
                    local cooldown = GetGameTimer()

                    local model = GetHashKey(ConfigLOAF.Furniture[category[1]][current][2])
                    ...
                    if IsModelValid(model) then
                        ...
                        while not HasModelLoaded(model) do 
                            ...
                            if GetGameTimer() - startedLoading >= 1500 then
                                ESX.ShowNotification(('The model (%s) is taking a looong time to load. Contact the server owner.'):format(ConfigLOAF.Furniture[category[1]][current][2]))
                                break
                            end
                        end
                        furniture = CreateObject(model, ConfigLOAF.Furnituring['teleport'])
                        SetEntityHeading(furniture, 0.0)
                    else
                        ESX.ShowNotification(('The model (%s) isn\'t valid. Contact the server owner.'):format(ConfigLOAF.Furniture[category[1]][current][2]))
                    end
                    ...
                    while true do
                        Wait(0)
                        HelpText(('~INPUT_FRONTEND_LEFT~ ~INPUT_FRONTEND_RIGHT~ browse\n~INPUT_CELLPHONE_DOWN~ ~INPUT_CELLPHONE_UP~ change category (%s)\n~INPUT_FRONTEND_RDOWN~ buy %s for $%s\n~INPUT_MOVE_UP_ONLY~ ~INPUT_MOVE_DOWN_ONLY~ ~INPUT_VEH_CINEMATIC_UP_ONLY~ ~INPUT_VEH_CINEMATIC_DOWN_ONLY~ adjust camera\n~INPUT_MOVE_LEFT_ONLY~ ~INPUT_MOVE_RIGHT_ONLY~ rotate\n\n~INPUT_FRONTEND_RRIGHT~ exit'):format(category[2], ConfigLOAF.Furniture[category[1]][current][1], ConfigLOAF.Furniture[category[1]][current][3]))

                        DrawText3D(GetEntityCoords(furniture), ('%s (~g~$%s~w~)'):format(ConfigLOAF.Furniture[category[1]][current][1], ConfigLOAF.Furniture[category[1]][current][3]), 0.5)
                        ...
                        elseif IsControlJustReleased(0, 172) then
                            if ConfigLOAF.Furniture['Categories'][currentcategory + 1] then
                                category = ConfigLOAF.Furniture['Categories'][currentcategory + 1]
                                currentcategory = currentcategory + 1
                                current = 1
                            else
                                category = ConfigLOAF.Furniture['Categories'][1]
                                currentcategory = 1
                                current = 1
                            end
                            DeleteObject(furniture)
                            model = GetHashKey(ConfigLOAF.Furniture[category[1]][current][2])
                            if IsModelValid(model) then
                                local startedLoading = GetGameTimer()
                                while not HasModelLoaded(model) do 
                                    RequestModel(model) Wait(0) 
                        ...
                        elseif IsControlJustReleased(0, 173) then
                            if ConfigLOAF.Furniture['Categories'][currentcategory - 1] then
                                ...
                                category = ConfigLOAF.Furniture['Categories'][#ConfigLOAF.Furniture['Categories']]
                                currentcategory = #ConfigLOAF.Furniture['Categories']
                                current = 1
                            end
                            DeleteObject(furniture)
                            model = GetHashKey(ConfigLOAF.Furniture[category[1]][current][2])
                        ...
                        elseif IsControlJustReleased(0, 191) then
                            ...
                            {
                                title = ('Buy %s for $%s?'):format(ConfigLOAF.Furniture[category[1]][current][1], ConfigLOAF.Furniture[category[1]][current][3]),
                                align = 'center',
                                elements = {
                                    {label = 'yes', value = 'yes'},
                                    {label = 'no', value = 'no'}
                        ...
                        elseif IsControlPressed(0, 190) and cooldown < GetGameTimer() then
                            if ConfigLOAF.Furniture[category[1]][current + 1] then
                                current = current + 1
                            else
                                current = 1
                            end
                        ...
                        elseif IsControlPressed(0, 189) and cooldown < GetGameTimer() then
                            if ConfigLOAF.Furniture[category[1]][current - 1] then
                                ...
                                current = #ConfigLOAF.Furniture[category[1]]
                            end
                            DeleteObject(furniture)
                            model = GetHashKey(ConfigLOAF.Furniture[category[1]][current][2])
                            if IsModelValid(model) then
...
AddEventHandler('loaf_housing:spawnHouse', function(coords, furniture)
    local prop = ConfigLOAF.Houses[OwnedHouse.houseId]['prop']
    local house = EnterHouse(ConfigLOAF.Props[prop], coords)
    ...
    local exit = GetOffsetFromEntityInWorldCoords(house, ConfigLOAF.Offsets[prop]['door'])
  ...
  NetworkOverrideClockTime(15, 0, 0)
        ...
        if Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
            HelpText(('Press ~INPUT_CONTEXT~ to %s'):format('access your storage'), storage)
        ...
        if Vdist2(GetEntityCoords(PlayerPedId()), exit) <= 1.5 then
            HelpText(('Press ~INPUT_CONTEXT~ to %s'):format('manage door'), exit)
            if IsControlJustReleased(0, 38) then
                ESX.UI.Menu.CloseAll()

                local elements = {
                    {label = 'Accept', value = 'accept'},
                    {label = 'Furnish', value = 'furnish'},
                    {label = 'Re Furnish', value = 'refurnish'},
                    ...
                    if data.current.value == 'exit' then
                        ESX.TriggerServerCallback('loaf_housing:hasGuests', function(has)
                            if not has then
                                ...
                                ESX.ShowNotification('Your guests need to leave the house before you may do this.')
                            end
                        end)
                    elseif data.current.value == 'refurnish' then
                        ESX.TriggerServerCallback('loaf_housing:hasGuests', function(has)
                            if not has then
                                local elements = {}
                                for k, v in pairs(OwnedHouse['furniture']) do
                                    table.insert(elements, {label = ('Remove %s (#%s)'):format(v['name'], k), value = k})
                                end
                                local editing = true
                                ESX.UI.Menu.Open(
                            ...
                            if not has then
                                ...
                                function(data2, menu2)
                                    ...
                                    while true do
                                        ...
                                        DrawText3D(GetEntityCoords(object), ('%s, %s: %s'):format(data2.current.name,'Rotation', string.format("%.2f", GetEntityHeading(object))), 0.3)
...
AddEventHandler('loaf_housing:leaveHouse', function(house)
    DoScreenFadeOut(750)
    while not IsScreenFadedOut() do Wait(0) end
    SetEntityCoords(PlayerPedId(), ConfigLOAF.Houses[house]['door'])
    for i = 1, 25 do
        SetEntityCoords(PlayerPedId(),  ConfigLOAF.Houses[house]['door'])
        Wait(50)
    end
...
AddEventHandler('loaf_housing:knockAccept', function(coords, house, storage, spawnpos, furniture, host)
    local prop = ConfigLOAF.Houses[house]['prop']
    prop = EnterHouse(ConfigLOAF.Props[prop], spawnpos)
    local placed_furniture = {}
    for k, v in pairs(furniture) do
        local model = GetHashKey(v['object'])
        while not HasModelLoaded(model) do RequestModel(model) Wait(0) end
    ...
    while true do
        ...
        if delete then 
            ...
            DeleteObject(prop)
            DoScreenFadeIn(1500)
            ESX.ShowNotification('The house owner left the server, you were kicked out of their house.')
            return
        end
        ...
        if Vdist2(GetEntityCoords(PlayerPedId()), coords) <= 1.5 then
            HelpText(('Press ~INPUT_CONTEXT~ to %s'):format('Leave the house'), coords)
            if IsControlJustReleased(0, 38) then
                ESX.UI.Menu.CloseAll()
                DoScreenFadeOut(750)
                for k, v in pairs(placed_furniture) do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
            HelpText(('Press ~INPUT_CONTEXT~ to %s'):format('Manage Storage'), storage)
            if IsControlJustReleased(0, 38) and Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
                ESX.UI.Menu.CloseAll()
...
AddEventHandler('loaf_housing:knockedDoor', function(src)
    ESX.ShowNotification('Someone knocked. Go to the door to let them in!')
    if not Knockings[src] then
        Knockings[src] = {label = ('Let %s in?'):format(GetPlayerName(GetPlayerFromServerId(src))), value = src}
    end
end)
...
AddEventHandler('loaf_housing:setHouse', function(house, purchasedHouses)
    ...
    for k, v in pairs(ConfigLOAF.Houses) do
        if OwnedHouse.houseId == k then
            CreateBlip(v['door'], 40, 3, 0.75, 'Your house')
        else
            if not AvailableHouses[k] then
                if ConfigLOAF.AddHouseBlips then
                    CreateBlip(v['door'], 374, 0, 0.45, '')
                end
            else
                if ConfigLOAF.AddBoughtHouses then
                    CreateBlip(v['door'], 374, 2, 0.45, 'Player House')
                end
            end
        end
    end
...
HelpText = function(msg, coords)
    if not coords or not ConfigLOAF.Use3DText then
