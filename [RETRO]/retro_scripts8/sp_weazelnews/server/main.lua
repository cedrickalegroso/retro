...
if ConfigWeazel.MaxInService ~= -1 then
  TriggerEvent('esx_service:activateService', 'weazelnews', ConfigWeazel.MaxInService)
end
...
AddEventHandler('SyncWerbung', function(inputText)
	if os.time(os.date("!*t")) > (lastAdDate + 1800) then
		...
		TriggerClientEvent('DisplayWerbung', -1, inputText, xPlayer.getName())
				PerformHttpRequest(ConfigWeazel.Webhook, function(e,r,h) end, "POST", json.encode({
