			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'vermillion')

						TriggerServerEvent('esx_service:disableService', 'vermillion')
						TriggerEvent('retro_vermillion:updateBlip')
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 2.0 then
					...
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('retro_vermillion:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('retro_vermillion:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('retro_vermillion:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('retro_vermillion:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('retro_vermillion:getFineList', function(fines)
		local elements = {}
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('retro_vermillion:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('retro_vermillion:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('retro_vermillion:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('retro_vermillion:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('retro_vermillion:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('retro_vermillion:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('retro_vermillion:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('retro_vermillion:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if not count then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('retro_vermillion:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('retro_vermillion:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if not count then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('retro_vermillion:putStockItems', itemName, count)
...
AddEventHandler('retro_vermillion:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('retro_vermillion:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('retro_vermillion:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('retro_vermillion:handcuff')
AddEventHandler('retro_vermillion:handcuff', function()
...
RegisterNetEvent('retro_vermillion:unrestrain')
AddEventHandler('retro_vermillion:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('retro_vermillion:drag')
AddEventHandler('retro_vermillion:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('retro_vermillion:putInVehicle')
AddEventHandler('retro_vermillion:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('retro_vermillion:OutVehicle')
AddEventHandler('retro_vermillion:OutVehicle', function()
	local playerPed = PlayerPedId()
...
-- Create blips
Citizen.CreateThread(function()
	for k,v in pairs(Config.VermillionStations) do
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job2.name == 'vermillion' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('retro_vermillion:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('retro_vermillion:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('retro_vermillion:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('retro_vermillion:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
RegisterNetEvent('retro_vermillion:updateBlip')
AddEventHandler('retro_vermillion:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('retro_vermillion:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('retro_vermillion:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('retro_vermillion:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'vermillion')
