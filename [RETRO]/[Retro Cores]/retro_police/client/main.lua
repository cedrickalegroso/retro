			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('retro_police:updateBlip')
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = 'Soft Cuff',		value = 'softhandcuff'},
			...
			--	if closestPlayer ~= -1 and closestDistance <= 2.0 then
				...
				--		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 2.0, 'cuff', 0.7)
						TriggerServerEvent('retro_police:handcuff',  GetPlayerServerId(closestPlayer))
					...
					elseif action == 'softhandcuff' then	
						print('soft handcuff')					
					TriggerServerEvent('retro_police:softhandcuff',  GetPlayerServerId(closestPlayer))
					...
					elseif action == 'drag' then
						TriggerServerEvent('retro_police:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('retro_police:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('retro_police:OutVehicle', GetPlayerServerId(closestPlayer))
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = 'SSSSS', model = 'p_ld_stinger_s'},
	...
	ESX.TriggerServerCallback('retro_police:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('retro_police:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('retro_police:getFineList', function(fines)
		local elements = {}
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('retro_police:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('retro_police:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('retro_police:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('retro_police:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('retro_police:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('retro_police:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('retro_police:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('retro_police:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if not count then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('retro_police:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('retro_police:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if not count then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('retro_police:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('retro_police:forceBlip')
end)
...
AddEventHandler('retro_police:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('retro_police:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('retro_police:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('retro_police:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('retro_police:softhandcuff')
AddEventHandler('retro_police:softhandcuff', function()
...
RegisterNetEvent('retro_police:handcuff')
AddEventHandler('retro_police:handcuff', function()
	isHandcuffed = not isHandcuffed
	local playerPed = PlayerPedId()
...
RegisterNetEvent('retro_police:unrestrain')
AddEventHandler('retro_police:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('retro_police:drag')
AddEventHandler('retro_police:drag', function(copId)
...
RegisterNetEvent('retro_police:putInVehicle')
AddEventHandler('retro_police:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('retro_police:OutVehicle')
AddEventHandler('retro_police:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.policeStations) do
		...
		AddTextComponentSubstringPlayerName('police HERMANOS')
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('retro_police:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('retro_police:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('retro_police:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('retro_police:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
RegisterNetEvent('retro_police:updateBlip')
AddEventHandler('retro_police:updateBlip', function()

	print('show police blips')
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('retro_police:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('retro_police:spawned')
	end
