...
this.updateVehicle = function(data) {
	...
	if (data.status == true) {	
		...
		vehicleDamage.querySelector('span').style.height = data.damage+'%';

		if (data.damage <= 35) {
			if (vehicleDamage.classList.contains('dying') == false) { vehicleDamage.classList.add('dying');	}
		}
		else { vehicleDamage.classList.remove('dying'); }
