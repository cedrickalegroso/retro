			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'gordo')

						TriggerServerEvent('esx_service:disableService', 'gordo')
						TriggerEvent('retro_gordo:updateBlip')
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('retro_gordo:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('retro_gordo:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('retro_gordo:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('retro_gordo:getFineList', function(fines)
		local elements = {}
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('retro_gordo:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('retro_gordo:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('retro_gordo:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('retro_gordo:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('retro_gordo:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('retro_gordo:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('retro_gordo:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('retro_gordo:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if not count then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('retro_gordo:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('retro_gordo:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if not count then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('retro_gordo:putStockItems', itemName, count)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'gordo' and ESX.PlayerData.job2.name == dispatchNumber then
...
AddEventHandler('retro_gordo:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('retro_gordo:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('retro_gordo:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'gordo' and IsPedOnFoot(playerPed) then
...
AddEventHandler('retro_gordo:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('retro_gordo:handcuff')
AddEventHandler('retro_gordo:handcuff', function()
...
RegisterNetEvent('retro_gordo:drag')
AddEventHandler('retro_gordo:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('retro_gordo:putInVehicle')
AddEventHandler('retro_gordo:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('retro_gordo:OutVehicle')
AddEventHandler('retro_gordo:OutVehicle', function()
	local playerPed = PlayerPedId()
...
-- Create blips
Citizen.CreateThread(function()
	for k,v in pairs(Config.GordoStations) do
		...
		AddTextComponentSubstringPlayerName('gordo Mansion')
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job2.name == 'gordo' then
			local playerPed = PlayerPedId()
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('retro_gordo:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('retro_gordo:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('retro_gordo:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('retro_gordo:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'gordo' then
		...
		if IsControlJustReleased(0, 168) and not isDead and ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'gordo' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			if not Config.EnableESXService then
...
RegisterNetEvent('retro_gordo:updateBlip')
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('retro_gordo:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('retro_gordo:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('retro_gordo:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'gordo')
