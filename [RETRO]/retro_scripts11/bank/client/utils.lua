...
AddEventHandler('t1ger_bankrobbery:PoliceNotifyBlip', function(targetCoords)
	if isCop and ConfigBankrob.AlertBlipShow then 
		local alpha = ConfigBankrob.AlertBlipAlpha
		local alertBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, ConfigBankrob.AlertBlipRadius)

		SetBlipHighDetail(alertBlip, true)
		SetBlipColour(alertBlip, ConfigBankrob.AlertBlipColor)
		SetBlipAlpha(alertBlip, alpha)
		SetBlipAsShortRange(alertBlip, true)

		while alpha ~= 0 do
			Citizen.Wait(ConfigBankrob.AlertBlipTime * 4)
			alpha = alpha - 1
			SetBlipAlpha(alertBlip, alpha)
...
-- Bank Blips:
Citizen.CreateThread(function()
	for k,v in pairs(ConfigBankrob.Banks) do
		CreateMapBlip(k,v)
	end
end)
...
function IsPlayerJobCop()	
	...
	for k,v in pairs(ConfigBankrob.PoliceJobs) do
