...
RegisterNetEvent('t1ger_drugbusiness:ShowAdvancedNotifyESX')
AddEventHandler('t1ger_drugbusiness:ShowAdvancedNotifyESX', function(title, subject, msg, icon, iconType)
...
RegisterNetEvent('t1ger_drugbusiness:ShowNotifyESX')
AddEventHandler('t1ger_drugbusiness:ShowNotifyESX', function(msg)
	ShowNotifyESX(msg)
end)
...
function NotifyPoliceFunction()
	TriggerServerEvent('t1ger_drugbusiness:PoliceNotifySV',GetEntityCoords(GetPlayerPed(-1)),streetName)
...
RegisterNetEvent('t1ger_drugbusiness:PoliceNotifyCL')
AddEventHandler('t1ger_drugbusiness:PoliceNotifyCL', function(alert)
	if isCop then
...
RegisterNetEvent('t1ger_drugbusiness:PoliceNotifyBlip')
AddEventHandler('t1ger_drugbusiness:PoliceNotifyBlip', function(targetCoords)
	if isCop and ConfigDRUGBUSI.AlertBlipShow then 
		local alpha = ConfigDRUGBUSI.AlertBlipAlpha
		local alertBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, ConfigDRUGBUSI.AlertBlipRadius)

		SetBlipHighDetail(alertBlip, true)
		SetBlipColour(alertBlip, ConfigDRUGBUSI.AlertBlipColor)
		SetBlipAlpha(alertBlip, alpha)
		SetBlipAsShortRange(alertBlip, true)

		while alpha ~= 0 do
			Citizen.Wait(ConfigDRUGBUSI.AlertBlipTime * 4)
			alpha = alpha - 1
			SetBlipAlpha(alertBlip, alpha)
...
function CreateLabBlips(k,v,label)
	local mk = ConfigDRUGBUSI.BlipSettings 
	if mk.enable then
		...
		if label == "Your Lab" then
			SetBlipColour (blip, 2)
		elseif label == "Player Lab" then
...
function IsPlayerJobCop()	
	...
	for k,v in pairs(ConfigDRUGBUSI.PoliceJobs) do
