...
AddEventHandler('t1ger_drugbusiness:productionTime', function()
    ...
    while ownsLab do
        Citizen.Wait((ConfigDRUGBUSI.ProductionMinutes * 1000 * 60))
...
AddEventHandler('t1ger_drugbusiness:applyPlyLabID', function(labID)
	...
	ESX.TriggerServerCallback('t1ger_drugbusiness:getTakenLabs', function(takenLabs)
		...
		for k,v in pairs(ConfigDRUGBUSI.DrugLabs) do
			if plyLabID == k then
				if ConfigDRUGBUSI.OwnedLabBlip  then
					CreateLabBlips(k,v,"Your Lab")
				end
			else
				if emptyLabs[k] == k then
					if ConfigDRUGBUSI.PlayerLabsBlip then
						CreateLabBlips(k,v,"Player Lab")
					end
				else
					if ConfigDRUGBUSI.PurchasableLabBlip then
	...
	while true do
		...
		for k,v in pairs(ConfigDRUGBUSI.DrugLabs) do
			...
			if labMenu ~= nil then
				...
				local mk = ConfigDRUGBUSI.MarkerSettings
                ...
                elseif distance <= 2.0 then
                    ...
                    if plyLabID == k then
                        ...
                        if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToManageLab) then
							labMenu = v
                        ...
                        if emptyLabs[k] == k then
							if isCop then
								...
								if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToRaidLab) then
									...
									--exports['progressBars']:startUI(((ConfigDRUGBUSI.WaitTimeUntilRaid * 1000)), Lang['prog_bar_raid_door'])
									Citizen.Wait((ConfigDRUGBUSI.WaitTimeUntilRaid * 1000))
									Wait(150)
								...
								if plyLabID ~= 0 then
									...
									if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToRobLab) and not hacking then
										hacking = true
										GainAccessToLab(k,v)
									end
								else
                            ...
                            if plyLabID == 0 then
                                ...
                                if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToPurchaseLab) then
                                    labMenu = v
...
function GainAccessToLab(id,val)
	...
	--exports['progressBars']:startUI(((ConfigDRUGBUSI.WaitTimeUntilHack * 1000)), Lang['prog_bar_pre_hack'])
	Citizen.Wait((ConfigDRUGBUSI.WaitTimeUntilHack * 1000))
	TriggerEvent("mhacking:show")
	TriggerEvent("mhacking:start",ConfigDRUGBUSI.mHackingBlocks,ConfigDRUGBUSI.mHackingSeconds,hackingCallback)
end
...
function SellDrugLabMenu(id,val)
	local playerPed  = GetPlayerPed(-1)
	local sellPrice = (val.price * ConfigDRUGBUSI.SellPercent)
	local elements = {
...
function SpawnDrugLab(id,val)
    ...
    Citizen.Wait(2000)
    -- Spawning the shell:
    local prop = ConfigDRUGBUSI.DrugLabs[id].prop
    RequestModel(ConfigDRUGBUSI.ShellProps[prop])
    while not HasModelLoaded(ConfigDRUGBUSI.ShellProps[prop]) do
        Citizen.Wait(10)
	end
	local shell = CreateObject(ConfigDRUGBUSI.ShellProps[prop], shellPos[1], shellPos[2], shellPos[3], false, false, false)
    ...
    Citizen.Wait(500)
    -- Teleport Player:
    local offset = ConfigDRUGBUSI.Offsets[prop]
	...
	while insideShell do
		...
		if offset.laptop ~= nil then
			if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, laptop[1], laptop[2], laptop[3], false) <= 2.0 and not inLaptop then
				...
				if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToLaptop) and not inLaptop then
					inLaptop = true
		...
		if inDruglab ~= nil then
			...
			elseif distance <= 2.0 then
				...
				if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToLeaveLab) then
                    inDruglab = entry
                    local exit = ConfigDRUGBUSI.DrugLabs[id].pos
		...
		if(data.current.value == 'steal_supplies') then
			if not stealingSupplies then
				ESX.TriggerServerCallback('t1ger_drugbusiness:getSupplies', function(supplies)
					if supplies ~= nil then
						if supplies < 5 then
							local id = math.random(1,#ConfigDRUGBUSI.StealSupplies)
							local curID = 0
							while ConfigDRUGBUSI.StealSupplies[id].started and curID < 100 do
								curID = curID + 1
								id = math.random(1,#ConfigDRUGBUSI.StealSupplies)
							end
							if curID == 100 then
								ShowNotifyESX(Lang['no_jobs_available'])
							else
								stealingSupplies = true
								ConfigDRUGBUSI.StealSupplies[id].started = true
								Wait(200)
								TriggerServerEvent('t1ger_drugbusiness:JobDataSV',ConfigDRUGBUSI.StealSupplies)
...
function StockSellData() 
	...
	while not gotStock do
		...
		if gotStock and not gotMultiplier then
			for k,v in pairs(ConfigDRUGBUSI.SellMultiplier) do
	...
	local stockPrice = (ConfigDRUGBUSI.StockLevelPrice * stockLevel)
		...
		if(data.current.value == 'sell_stock_job') then
			if not sellingStock then 
				if stockLevel > 0 then 
					local id = math.random(1,#ConfigDRUGBUSI.SellStock)
					local curID = 0
					while ConfigDRUGBUSI.SellStock[id].started and curID < 100 do
						curID = curID + 1
						id = math.random(1,#ConfigDRUGBUSI.SellStock)
					end
					if curID == 100 then
						ShowNotifyESX(Lang['no_available_buyers'])
					else
						sellingStock = true
						ConfigDRUGBUSI.SellStock[id].started = true
						Wait(200)
						TriggerServerEvent('t1ger_drugbusiness:SellJobDataSV',ConfigDRUGBUSI.SellStock)
...
AddEventHandler('t1ger_drugbusiness:StockDelivery', function(id, stockLevel, stockValue)
	...
	local num = ConfigDRUGBUSI.SellStock[id]
	...
	while not soldStock and not endStockJob do
		...
		if num.started then
			...
			local startPos = ConfigDRUGBUSI.DrugLabs[plyLabID].delivery
			...
			if not stockVehSpawned then
				...
				Citizen.Wait(200)
				stockVehModel = ConfigDRUGBUSI.StockSellVeh
			...
			if not stockVehDelivered then
				...
				local mk = ConfigDRUGBUSI.DeliveryMarker
				if mk.enable then
				...
				if distToDelivery <= 2.0 and not stockVehDelivered then
					...
					if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToDeliverStockVeh) then 
						if GetEntityModel(vehicle) == GetHashKey(stockVehModel) then
			...
			if endStockJob then
				...
				Wait(100)
				ConfigDRUGBUSI.SellStock[id].started = false
				Wait(200)
				TriggerServerEvent('t1ger_drugbusiness:SellJobDataSV',ConfigDRUGBUSI.SellStock)
				stockVehDelivered 	= false
				startPos			= nil
...
AddEventHandler('t1ger_drugbusiness:SuppliesJob', function(id)
	...
	local job = ConfigDRUGBUSI.StealSupplies[id]
	...
	while not gotSupplies and not endSupplyJob do
		...
		if job.started then
			...
			if distance < 150 and not jobVehSpawned then
				...
				jobVehSpawned = true
				Citizen.Wait(200)
				local num = math.random(1,#ConfigDRUGBUSI.JobVehicles)
				vehModel = ConfigDRUGBUSI.JobVehicles[num]
			...
			if distance < 60 and not job.JobPlayer then
				job.JobPlayer = true
				Wait(200)
				TriggerServerEvent('t1ger_drugs:JobDataSV',ConfigDRUGBUSI.Jobs)
				Citizen.Wait(200)
				SetPedRelationshipGroupHash(GetPlayerPed(-1), GetHashKey("PLAYER"))
				AddRelationshipGroup('JobNPCs')
				local i = 0
			...
			if distToVeh <= 2 and not jobVehStolen then
				...
				if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToLockpickJobVeh) and not curStealing then 
					StealJobVehicle(job)
				end
			end
			...
			if jobVehStolen and not jobVehDelivered then
				...
				if not endBlip then
					endBlip = true 
					endPos = ConfigDRUGBUSI.DrugLabs[plyLabID].delivery
			...
			if deliveryStage then
				...
				if distToDelivery <= 2.0 and not jobVehDelivered then
					...
					if IsControlJustPressed(0, ConfigDRUGBUSI.KeyToDeliverJobVeh) then 
			...
			if endSupplyJob then
				...
				Wait(100)
				ConfigDRUGBUSI.StealSupplies[id].started = false
				Wait(200)
				TriggerServerEvent('t1ger_drugbusiness:JobDataSV',ConfigDRUGBUSI.StealSupplies)
				goons 				= {}
				goonSpawn 			= false
...
function StealJobVehicle(val)	
	...
	if ConfigDRUGBUSI.NotifyCops then
