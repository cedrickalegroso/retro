...
Padlock.Close = function() {
    ...
    $.post('http://retro-storerob/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('http://retro-storerob/PadLockClose');
    if (data.error != null) {
        $.post('http://retro-storerob/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("http://retro-storerob/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        if (!$(".num" + (i + 1)).hasClass("found")) {
            if (angle > (comboArr[i] - numOffset) * tickAngle &&
                ...
                // on unlock
                $.post('http://retro-storerob/callcops');
                if (i == comboArr.length - 1) {
                    // unlock :)
                    $.post('http://retro-storerob/PadLockSuccess');
                    Padlock.Close();
                }
            }
            break;
...
function unlock() {
    gameOver = true;
    $.post('http://retro-storerob/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
