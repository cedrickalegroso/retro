...
RegisterNetEvent('esx_cichygps:lokalizator')
AddEventHandler('esx_cichygps:lokalizator', function()
...
local vehicle = ESX.Game.GetVehicleInDirection()
	 if DoesEntityExist(vehicle) then
							...
							AddTextComponentString('GPS siustuvas')
							EndTextCommandSetBlipName(oznaczenie)
										PlaySoundFrontend(-1, 'BACK', 'HUD_AMMO_SHOP_SOUNDSET', false)
								ESX.Scaleform.ShowFreemodeMessage('~o~GPS ATTACHED ', '~y~', 1)
								Citizen.Wait(3600000)
									    RemoveBlip(oznaczenie)
									else
	ESX.ShowHelpNotification('Netoliese nera jokios tr. priemones')
	end
