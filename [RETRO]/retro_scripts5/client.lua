...
Citizen.CreateThread(function()
	...
	for k, v in pairs(ConfigT.Shops) do
...
Citizen.CreateThread(function()
	...
	for k, v in ipairs(ConfigT.TattooCats) do
    ...
    while true do 
		...
		local CanSleep = true
		if not IsMenuOpen() then
			for _,interiorId in ipairs(ConfigT.interiorIds) do
		...
		for k, v in ipairs(ConfigT.TattooCats) do
			if JayMenu.IsMenuOpened(v[1]) then
				...
				for _, tattoo in pairs(ConfigT.AllTattooList) do
