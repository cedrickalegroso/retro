...
AddEventHandler('gln_oponazapasowa:uzyj', function()
	ESX.ShowHelpNotification('~r~Go to the nearest borken wheel')
	animacja()
end)
		...
		if vehicle ~= 0 then
			...
			if closestTire ~= nil then
				if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
					...
					Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, "~r~This tire needs to be repaired!")
				end
			end
		end
	end
		...
		if DoesEntityExist(vehicle) then
			...
			TriggerServerEvent('gln_oponazapasowa:usunOpone')
				...
				Citizen.CreateThread(function()
					...
					TriggerEvent("pNotify:SendNotification", {text = "You are in the process of replacing your tire!", type = "info", timeout = 5000, layout = "centerLeft"})
					...
					if CurrentAction ~= nil then
						...
						if closestTire ~= nil then
							...
							TriggerEvent("pNotify:SendNotification", {text = "You've finished replacing the tire!", type = "success", timeout = 2000, layout = "centerLeft"})
						end
					end
