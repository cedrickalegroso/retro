...
AddEventHandler("esx_trunk:getItem", function(plate, type, item, count, max, owned)
    ...
    if type == "item_standard" then
      ...
      print(targetItem.name)
      print(targetItem.count)

      local totalgg =  count + targetItem.count

      print('ayy '..totalgg)

      if targetItem.name == 'packaged_chicken' or targetItem.name == 'packaged_plank'  or targetItem.name == 'essence'  then 
               
                 if  totalgg > 100  then 
                  TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text ='You can carry 100 of these at a time' } )
                 else 
                  TriggerEvent("esx_trunk:getSharedDataStore", plate, function(store)
                    local coffre = (store.get("coffre") or {})
                    for i = 1, #coffre, 1 do
      ...
      else if targetItem.count <= targetItem.limit - 1 then
        print("SPACE FOR STANDARD")

        if  totalgg > 50  then 
          TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text ='You can carry 50 of these at a time' } )
        else
