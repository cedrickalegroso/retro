TriggerEvent("esx:addGroup", "mod", "user", function(group) end)
...
AddEventHandler("kc_admin:set_group", function (mod_source, target_id, group)
    ...
    if lowerGroup(group, local_group) then
        TriggerEvent("esx:setPlayerData", tonumber(target_id), "group", group, function(response, success)
            if success then
...
AddEventHandler("kc_admin:request_group", function ()
    ...
    if Source ~= nil then
        ...
        local player_steamid = Source.getIdentifier()
