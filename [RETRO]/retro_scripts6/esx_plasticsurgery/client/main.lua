...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(ConfigPlastic.Zones) do
			if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < ConfigPlastic.DrawDistance) then
				canSleep = false
...
function refreshBlips()
	if ConfigPlastic.EnableBlips then
		if ConfigPlastic.EnableUnemployedOnly then
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'unemployed' or ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'gang' then
				for k,v in pairs(ConfigPlastic.Locations) do
					local blip = AddBlipForCoord(v.x, v.y)

					SetBlipSprite (blip, ConfigPlastic.BlipPlasticSurgery.Sprite)
					SetBlipDisplay(blip, ConfigPlastic.BlipPlasticSurgery.Display)
					SetBlipScale  (blip, ConfigPlastic.BlipPlasticSurgery.Scale)
					SetBlipColour (blip, ConfigPlastic.BlipPlasticSurgery.Color)
			...
			for k,v in pairs(ConfigPlastic.Locations) do
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Plastic Surgery')
				EndTextCommandSetBlipName(blip)
				table.insert(BlipList, blip)
			end
		end
...
Citizen.CreateThread(function()
	...
	if ConfigPlastic.EnablePeds then
		for _, item in pairs(ConfigPlastic.Locations) do
...
RegisterNetEvent("retro_scripts:engine")
AddEventHandler("retro_scripts:engine", function(source)
	local v = GetVehiclePedIsIn(GetPlayerPed(-1))
    if GetIsVehicleEngineRunning(v) then
        exports['mythic_notify']:SendAlert('info', 'Engine Off', 2500)
        SetVehicleEngineOn(v, false, false)
    else
        exports['mythic_notify']:SendAlert('info', 'Engine On', 2500)
