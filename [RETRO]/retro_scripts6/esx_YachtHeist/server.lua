...
local YachtHeist 		= ConfigYacht.Yacht
...
ESX.RegisterServerCallback("esx_YachtHeist:GetPoliceOnline",function(source,cb)
	...
	for i = 1, #Players do
		...
		if xPlayer["job"]["name"] == ConfigYacht.PoliceDBname then
			policeOnline = policeOnline + 1
		end
	end
	if policeOnline >= ConfigYacht.RequiredPolice then
		cb(true)
	else
		cb(false)
	end
...
RegisterServerEvent('nef_anncyacht:gg')
AddEventHandler('nef_anncyacht:gg', function()
...
local message = name..'started a yacht robbery'
local color = 56108
...
sendToDiscord (name,message,color, webhook)  
	...
	TriggerClientEvent('chat:addMessage', -1, {
		template = '<div class="chat-message system"><b> 👮 RCPD : </b> ^1Ongoing Yacht Robbery ^3 EMS AND MECHANIC STANDBY!  </div>',
		args = { fal, msg }
	})
...
AddEventHandler('esx_YachtHeist:safeRobbed', function(id, state)
	...
	local cashReward = 0
	local itemReward = 0
	if ConfigYacht.RewardBasedOnCops then
		cashReward = ((math.random(ConfigYacht.SafeMinCash,ConfigYacht.SafeMaxCash) * 1000) * policeReward)
		itemReward = ((math.random(ConfigYacht.SafeMinItem,ConfigYacht.SafeMaxItem) * 10) * policeReward)
	else
		cashReward = (math.random(ConfigYacht.SafeMinCash,ConfigYacht.SafeMaxCash) * 1000)
		itemReward = (math.random(ConfigYacht.SafeMinItem,ConfigYacht.SafeMaxItem) * 10)
	end
	xPlayer.addInventoryItem(ConfigYacht.SafeBoxItem,itemReward)
...
ESX.RegisterServerCallback("esx_YachtHeist:updateCashTaken",function(source,cb)
	...
	if xPlayer then
		local randomMoney = 0
		if ConfigYacht.RewardBasedOnCops then
			randomMoney = ((math.random(ConfigYacht.MinCashTake, ConfigYacht.MaxCashTake) * 100) * policeReward)
		else
			randomMoney = (math.random(ConfigYacht.MinCashTake, ConfigYacht.MaxCashTake) * 100)
		end
...
ESX.RegisterServerCallback("esx_YachtHeist:getHackerDevice",function(source,cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem(ConfigYacht.HackerDevice).count >= 1 then
		cb(true)
	else
		cb(false)
...
ESX.RegisterServerCallback("esx_YachtHeist:getDrillItem",function(source,cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem(ConfigYacht.DrillItem).count >= 1 then
		cb(true)
	else
		cb(false)
...
AddEventHandler("esx_YachtHeist:heistCooldown",function()
	table.insert(HeistTimer,{time = (ConfigYacht.CooldownTimer * 60000)}) -- cooldown timer for doing missions
end)
