...
AddEventHandler('nitro:check', function()
  ...
  local veh = GetVehiclePedIsIn(ped, false)
  local hash = GetEntityModel(veh)
  if GetEntitySpeed(veh)>0.1 then exports['mythic_notify']:DoHudText('error', 'Vehicle must be stopped!'); return end
  if veh and IsThisModelACar(hash) then
    TriggerServerEvent('nitro:removeInventoryItem','nitro', 1)
  else
    exports['mythic_notify']:DoHudText('error', 'You need to be in a vehicle to install this!')
  end
end)
...
AddEventHandler('nitro:activated', function()
  ...
  exports['mythic_notify']:DoHudText('success', 'Nitrous Installed')
  engineon = true
  nitro = 100
end)
