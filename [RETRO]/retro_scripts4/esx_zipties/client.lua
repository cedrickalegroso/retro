  ...
  Citizen.CreateThread(function()
    if not IsZiptied == true then
      ...
      FreezeEntityPosition(playerPed, false)
			...
			ESX.ShowAdvancedNotification('Criminal', '', 'Something Went Worng', 'CHAR_ARTHUR', 7)
    end
  end)
end)
			...
			if IsControlPressed(6, Keys["L"]) and IsControlPressed(0 , Keys['LEFTSHIFT']) then
				ESX.TriggerServerCallback('esx_zipties:getItemAmount', function(zipties)
					if zipties > 0 then
						...
						ESX.ShowAdvancedNotification('Criminal', '', 'You have no ~r~zipties', 'CHAR_ARTHUR', 7)
						ESX.UI.Menu.CloseAll()
						local elements = {}
...
function unZiptiePlayer()
	...
	if closestPlayer == -1 or closestDistance > 3.0 then
		...
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), "mp_arresting", "idle", 3) then
			...
			ESX.ShowAdvancedNotification('Criminal', '', 'Person is not ziptied', 'CHAR_ARTHUR', 7)
		end
	end
end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          TriggerServerEvent('', GetPlayerServerId(player), itemType, itemName, amount)
...
Citizen.CreateThread(function()
  local dict = "missminuteman_1ig_2"
	...
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(1, 48) then --Start holding X
      if not handsup then
