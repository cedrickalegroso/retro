...
Citizen.CreateThread(function()
	while true do
		...
		if (GetDistanceBetweenCoords(coords, 2441.460, 4755.190, 33.70, true) < 30.0)  then
					...
					ESX.Game.Utils.DrawText3D({ x = 2441.06, y = 4755.05, z = 33.85 }, '~y~[E] ~w~to Milk the cow', 0.6)		
					ESX.Game.Utils.DrawText3D({ x = 2443.96, y = 4764.95, z = 33.85 }, '~y~[E] ~w~to Milk the cow', 0.6)	
					ESX.Game.Utils.DrawText3D({ x = 2434.870, y = 4764.150, z = 33.80 }, '~y~[E] ~w~to Milk the cow', 0.6)				
					ESX.Game.Utils.DrawText3D({ x = 2430.76, y = 4773.95, z = 33.85 }, '~y~[E] ~w~to Milk the cow', 0.6)				
	...
	ESX.Game.Utils.DrawText3D({ x = 2512.870, y = 4761.850, z = 34.90 }, '~y~[E] ~w~Cloakroom', 0.6)	
			local coords      = GetEntityCoords(GetPlayerPed(-1))
						...
						if(GetDistanceBetweenCoords(coords, 2500.960, 4800.750, 34.740, true) < 3.0) then
							ESX.ShowAdvancedNotification('Pouring milk', '~y~Farma', '~y~[E] ~w~Pour milk into the machine', 'CHAR_PROPERTY_SONAR_COLLECTIONS', 3)
								if IsControlJustReleased(0, Keys['E']) then
                            		  skup()
								end
...
function skup()
	...
	TriggerServerEvent('smerfikcraft:skRETROup22')
...
local blips = {
	...
	{title="Krowy", colour=4, id=141, x = 2438.240, y = 4765.890, z = 35.00},
	{title="Przebieralnia mleczarzy", colour=4, id=366, x = 2512.990, y = 4762.750, z = 34.90},
	{title="Maszyna mleczarzy", colour=4, id=402, x = 2502.120, y = 4801.250, z = 43.740}
