...
Citizen.CreateThread(function()
	...
	for i=1, #Config.ShopsBAR, 1 do
		...
		local blip = AddBlipForCoord(Config.ShopsBAR[i].x, Config.ShopsBAR[i].y, Config.ShopsBAR[i].z)
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Config.ZonesBAR) do
			if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistanceBAR) then
