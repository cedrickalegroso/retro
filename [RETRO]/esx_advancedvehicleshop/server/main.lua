	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		if model == Config.AmbulanceHeli or model == Config.AmbulanceHeli2 then
			...
			MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, type, job) VALUES (@owner, @plate, @vehicle, @type, @job)', {
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'A vehicle with plate number '..plate..' now belongs to you.' })
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		if model == Config.PoliceHeli then
			...
			--	xPlayer.showNotification(_U('police_belongs', plate))
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'A car with plate '..plate..' now belongs to you.'})
				cb(true)
			end)
		end
	else
...
ESX.RegisterServerCallback('esx_advancedvehicleshop:buyVehicleA', function(source, cb, model, plate)
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, type) VALUES (@owner, @plate, @vehicle, @type)', {
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'A Aircraft with plate '..plate..' now belongs to you.'})
			cb(true)
		end)
	else
		cb(false)
...
ESX.RegisterServerCallback('esx_advancedvehicleshop:buyVehicleB', function(source, cb, model, plate)
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'A Boat with plate number '..plate..' now belongs to you.' })
			cb(true)
		end)
	else
		cb(false)
...
ESX.RegisterServerCallback('esx_advancedvehicleshop:buyVehicleC', function(source, cb, model, plate)
	...
	TriggerEvent('esx_society:depositTax',source, society, amount)
		...
		MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, type, tunerdata) VALUES (@owner, @plate, @vehicle, @type, @tunerdata)', {
			['@owner'] = xPlayer.identifier,
			['@plate'] = plate,
			['@vehicle'] = json.encode({model = GetHashKey(model), plate = plate}),
			['@type'] = 'car',
...
ESX.RegisterServerCallback('esx_advancedvehicleshop:retrieveJobVehicles', function(source, cb, type)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND type = @type AND job = @job', {
		...
		print(result[2].plate)
	end)
end)


ESX.RegisterServerCallback('esx_advancedvehicleshop:retrieveJobVehicles2', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('esx_advancedvehicleshop:setJobVehicleState2')
AddEventHandler('esx_advancedvehicleshop:setJobVehicleState2', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
