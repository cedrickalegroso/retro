...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedcuatroCars', function(source, cb)
	local ownedcuatroCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.Main.ShowVehLoc then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
			...
			for _,v in pairs(data) do
				local vehicle = json.decode(v.vehicle)
				table.insert(ownedcuatroCars, {vehicle = vehicle, stored = v.stored, plate = v.plate})
			end
			cb(ownedcuatroCars)
		end)
	else
...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedcuatroAircrafts', function(source, cb)
	local ownedcuatroAircrafts = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.Main.AdvVehShop then
		if Config.Main.ShowVehLoc then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedcuatroAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'aircraft'})
				end
				cb(ownedcuatroAircrafts)
			end)
		else
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', { -- job = NULL
				['@owner'] = xPlayer.identifier,
		...
		if Config.Main.ShowVehLoc then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedcuatroAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'helicopter'})
				end
				cb(ownedcuatroAircrafts)
			end)
		else
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedcuatroCars', function(source, cb)
	local ownedcuatroCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND job = @job AND `stored` = @stored', {
		...
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
			table.insert(ownedcuatroCars, vehicle)
		end
		cb(ownedcuatroCars)
	end)
end)

ESX.RegisterServerCallback('esx_advancedgarage:checkMoneycuatro', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getMoney() >= Config.Cuatro.PoundP then
		cb(true)
	else
		cb(false)
	end
...
RegisterServerEvent('esx_advancedgarage:paycuatro')
AddEventHandler('esx_advancedgarage:paycuatro', function()
	local xPlayer = ESX.GetPlayerFromId(source)

	--[[ 
xPlayer.removeMoney(Config.cuatro.PoundP)
	TriggerClientEvent('esx:showNotification', source, _U('you_paid') .. Config.cuatro.PoundP)

	if Config.Main.GiveSocMoney then
		TriggerEvent('esx_addonaccount:getSharedAccount', 'society_groove', function(account)
			account.addMoney(Config.cuatro.PoundP)
		end)
	end
...
-- Start of vermillion Code
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedvermillionCars', function(source, cb)
	local ownedvermillionCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.Main.ShowVehLoc then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
			...
			for _,v in pairs(data) do
				local vehicle = json.decode(v.vehicle)
				table.insert(ownedvermillionCars, {vehicle = vehicle, stored = v.stored, plate = v.plate})
			end
			cb(ownedvermillionCars)
		end)
	else
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', { -- job = NULL
			['@owner'] = xPlayer.identifier,
...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedvermillionAircrafts', function(source, cb)
	local ownedvermillionAircrafts = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.Main.AdvVehShop then
		if Config.Main.ShowVehLoc then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedvermillionAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'aircraft'})
				end
				cb(ownedvermillionAircrafts)
			end)
		else
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', { -- job = NULL
				['@owner'] = xPlayer.identifier,
		...
		if Config.Main.ShowVehLoc then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedvermillionAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'helicopter'})
				end
				cb(ownedvermillionAircrafts)
			end)
		else
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedvermillionCars', function(source, cb)
	local ownedvermillionCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND job = @job AND `stored` = @stored', {
		...
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
			table.insert(ownedvermillionCars, vehicle)
		end
		cb(ownedvermillionCars)
	end)
end)

ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyvermillion', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getMoney() >= Config.Vermillion.PoundP then
		cb(true)
	else
		cb(false)
	end
...
RegisterServerEvent('esx_advancedgarage:payvermillion')
AddEventHandler('esx_advancedgarage:payvermillion', function()
	local xPlayer = ESX.GetPlayerFromId(source)

	--[[ 
xPlayer.removeMoney(Config.vermillion.PoundP)
	TriggerClientEvent('esx:showNotification', source, _U('you_paid') .. Config.vermillion.PoundP)

	if Config.Main.GiveSocMoney then
		TriggerEvent('esx_addonaccount:getSharedAccount', 'society_groove', function(account)
			account.addMoney(Config.vermillion.PoundP)
		end)
	end
...
AddEventHandler('esx_advancedgarage:payCar', function()
	...
	local society = 'government'
		local amount  = Config.Cars.PoundP * 0.05
