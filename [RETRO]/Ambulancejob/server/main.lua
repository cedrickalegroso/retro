...
RegisterServerEvent('retro_scripts:clearplayerloadoutitems')
AddEventHandler('retro_scripts:clearplayerloadoutitems', function(ttPID)

	local xPlayer

	if ttPID then
		xPlayer = ESX.GetPlayerFromId(ttPID)
	else
...
RegisterCommand("newlife", function(source, args, user)
...
if not xPlayer then
		TriggerClientEvent('chat:addMessage', source, { args = { '^1CEDRICK_SCRIPTS', 'Player not online.' } })
		return
	end
	
	

	TriggerClientEvent('chat:addMessage', source, { args = { '^1CEDRICK_SCRIPTS', ' Player is online! sending to purgatory' } })
	
	--print(args[1])

    TriggerClientEvent("retro_scripts:newlife2", args[1], source)
...
AddEventHandler('AnnounceEMSOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~EMS announcement', 'An EMS is in service! Your health first!', 'CHAR_CALL911', 8)
	end
end)
...
AddEventHandler('AnnounceEMSFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~EMS announcement', 'There are no more EMS in service!', 'CHAR_CALL911', 8)
	end
end)
