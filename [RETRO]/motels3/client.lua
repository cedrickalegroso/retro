          ...
          if not Config.DrawTextInsteadOfMarker then
            if doorDist < Config.DrawDist then
              ...
              if doorDist < Config.ActDist then
                ESX.ShowHelpNotification("Press ~INPUT_PICKUP~ to interaction.")
                if IsControlJustPressed(0, 38) then
            ...
            if doorDist < Config.DrawDist then
              utils.drawText3D(closestDoor,"Press [~r~E~s~] to interaction.")
              if doorDist < Config.ActDist then
                if IsControlJustPressed(0, 38) then
                  motels.doorLoop(closestDoor,closestZone)
                end
      ...
      elseif motels.ownedRooms[door] then
        ...
        if _owner then
          if motels.plyData and motels.plyData.job and motels.plyData.job.name == Config.PoliceJobName then
            if Config.DrawTextInsteadOfMarker then
              utils.drawText3D(door,"Press [~r~G~s~] to knock the door.\nPulsa [~r~H~s~] to break the door down.")
            else
              ESX.ShowHelpNotification("Press ~INPUT_DETONATE~ to ring the bell.\nPress ~INPUT_VEH_HEADLIGHT~ to break the door down.")
            end
            if IsControlJustPressed(0, 47) then
            ...
            if Config.DrawTextInsteadOfMarker then
              utils.drawText3D(door,"Press [~r~G~s~] to ring the bell.")
            else
              ESX.ShowHelpNotification("Press ~INPUT_DETONATE~ to ring the bell.")
            end
            if IsControlJustPressed(0, 47) then
...
motels.knockOnDoor = function(door,zone)
  ...
  ESX.ShowNotification("Has tocado el timbre.")
end
...
motels.hasKnocked = function(door,id)
  ...
  if motels.curRoom.entry and motels.curRoom.entry == door then
    ESX.ShowNotification("Someone knocked on the door. Use /invin ID to let him in. [ID:~g~"..id..'~s~]')
  end
end
...
motels.inviteIn = function(source,args)
  ...
  elseif not args or not args[1] then
    ESX.ShowNotification("Write an ID next time.")
  else
...
motels.inviteInside = function(door,spawn)
  ...
  if dist < 20.0 then
    Citizen.CreateThread(function(...)
      ...
      ESX.ShowNotification("You have been invited inside. Press [~ g ~ H ~ s ~] to accept.")
...
motels.getClosestAction = function(room)
  ...
  if not closestDist or inventDist < closestDist then
    ...
    closest = "inventory"
    actText = "Press "..(Config.DrawTextInsteadOfMarker and "[~r~E~s~]" or "~INPUT_PICKUP~").." to open inventory."
  end
...
RegisterNetEvent('motels:wardrobe')
AddEventHandler('motels:wardrobe', function()
  motels.wardrobeMenu()
end)
...
motels.buyRoom = function(door,zone)
  ESX.TriggerServerCallback('motels:tryBuy', function(canBuy,price)
    if canBuy then
      ...
      ESX.ShowNotification("You bought this room.")
    else
      ESX.ShowNotification("You cant afford this room")
    end
  end,zone,door)
end
...
motels.sellRoom = function(door,zone)
  ...
  ESX.ShowNotification("You have sold the room.")
...
motels.wardrobeMenu = function()     
  local elements = {}
  elements[1] = {label = "Change Outfit",val="Change"}
