...
local YachtHeist 		= ConfigYa.Yacht
...
ESX.RegisterServerCallback("esx_YachtHeist:GetPoliceOnline",function(source,cb)
	...
	for i = 1, #Players do
		...
		if xPlayer["job"]["name"] == ConfigYa.PoliceDBname then
			policeOnline = policeOnline + 1
		end
	end
	if policeOnline >= ConfigYa.RequiredPolice then
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_YachtHeist:safeRobbed', function(id, state)
	...
	local cashReward = 0
	local itemReward = 0
	if ConfigYa.RewardBasedOnCops then
		cashReward = ((math.random(ConfigYa.SafeMinCash,ConfigYa.SafeMaxCash) * 1000) * policeReward)
		itemReward = ((math.random(ConfigYa.SafeMinItem,ConfigYa.SafeMaxItem) * 10) * policeReward)
	else
		cashReward = (math.random(ConfigYa.SafeMinCash,ConfigYa.SafeMaxCash) * 1000)
		itemReward = (math.random(ConfigYa.SafeMinItem,ConfigYa.SafeMaxItem) * 10)
	end
	xPlayer.addInventoryItem(ConfigYa.SafeBoxItem,itemReward)
...
ESX.RegisterServerCallback("esx_YachtHeist:updateCashTaken",function(source,cb)
	...
	if xPlayer then
		local randomMoney = 0
		if ConfigYa.RewardBasedOnCops then
			randomMoney = ((math.random(ConfigYa.MinCashTake, ConfigYa.MaxCashTake) * 100) * policeReward)
		else
			randomMoney = (math.random(ConfigYa.MinCashTake, ConfigYa.MaxCashTake) * 100)
		end
...
ESX.RegisterServerCallback("esx_YachtHeist:getHackerDevice",function(source,cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem(ConfigYa.HackerDevice).count >= 1 then
		cb(true)
	else
		cb(false)
...
ESX.RegisterServerCallback("esx_YachtHeist:getDrillItem",function(source,cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem(ConfigYa.DrillItem).count >= 1 then
		cb(true)
	else
		cb(false)
...
AddEventHandler("esx_YachtHeist:heistCooldown",function()
	table.insert(HeistTimer,{time = (ConfigYa.CooldownTimer * 60000)}) -- cooldown timer for doing missions
end)
