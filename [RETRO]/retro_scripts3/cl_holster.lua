...
ConfigHolster 				  = {}
ConfigHolster.CooldownPolice = 700
ConfigHolster.cooldown		  = 1700

-- Add/remove weapon hashes here to be added for holster checks.
ConfigHolster.Weapons = {
	"WEAPON_PISTOL",
	"WEAPON_COMBATPISTOL",
	"WEAPON_APPISTOL",
	"WEAPON_PISTOL50",
						...
						if holstered then
							blocked   = true
								...
								--TaskPlayAnim(ped, "reaction@intimidation@cop@unarmed", "intro", 8.0, 2.0, -1, 30, 2.0, 0, 0, 0 ) Use this line if you want to stand still when removing weapon
									Citizen.Wait(ConfigHolster.CooldownPolice)
						...
						if holstered then
							blocked   = true
								...
								--TaskPlayAnim(ped, "reaction@intimidation@1h", "intro", 5.0, 1.0, -1, 30, 0, 0, 0, 0 ) Use this line if you want to stand still when removing weapon
									Citizen.Wait(1250)
								SetPedCurrentWeaponVisible(ped, 1, 1, 1, 1)
									Citizen.Wait(ConfigHolster.cooldown)
								ClearPedTasks(ped)
							holstered = false
						else
							blocked = false
...
function CheckWeapon(ped)
	...
	if IsEntityDead(ped) then
		blocked = false
			...
			for i = 1, #ConfigHolster.Weapons do
				if GetHashKey(ConfigHolster.Weapons[i]) == GetSelectedPedWeapon(ped) then
					return true
				end
			end
		return false
