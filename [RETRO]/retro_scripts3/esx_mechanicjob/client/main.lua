...
function OpenMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = _U('Nissan'), value = 'nissantitan17'}
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBosRETROsMenu', 'mechanic', function(data, menu)
				menu.close()
			end)
		end
	end, function(data, menu)
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('esx_mechanicjob:starRETROtCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('esx_mechanicjob:starRETROtCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('esx_mechanicjob:starRETROtCraft3')
			end
		end, function(data, menu)
			menu.close()
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = '🔧 Fix Wheel',        value = 'fix_wheel'},
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_biRETROlling:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
		...
		elseif data.current.value == 'fix_vehicle1' then
			TriggerServerEvent('esx_mechanicjob:checkFixtool')	
			Citizen.Wait(1000)


			
		elseif data.current.value == 'fix_wheel' then
		...
		elseif data.current.value == 'fix_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					TriggerServerEvent('esx_mechanicjob:checkFixtool2')	
					Citizen.Wait(1000)
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleRETROInGarage', 'mechanic', vehicleProps)
						...
						if
							...
							GetEntityModel(vehicle) == GetHashKey('nissantitan17') or
