...
RegisterNUICallback('TestDrive', function(data, cb) 
	...
	while counter > 0 do 
			--	exports['mythic_notify']:DoCustomHudText('branco', 'Test drive ends in: '.. counter .. ' seconds!',700)
		    	--ESX.ShowNotification('Test drive ends in: '.. counter .. ' seconds!')
				counter = counter -1
				Citizen.Wait(1000)
			end
...
RegisterNUICallback('BuyVehicle', function(data, cb)
	...
	ESX.ShowNotification('Vehicle Loaded')
	...
	elseif zone == 'ResellVehicle' then
		...
		if IsPedSittingInAnyVehicle(playerPed) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				...
				CurrentActionMsg  = 'Press ~g~[E]~w~ to sell for ' .. ESX.Math.GroupDigits(resellPrice)
				ESX.ShowHelpNotification('Press ~g~[E]~w~ to sell for ' .. ESX.Math.GroupDigits(resellPrice)..'€' )
		...
		for k,v in pairs(Config.Zones) do
			if v.Type == 36 then
				if Vdist(coords.x,coords.y,coords.z, v.Pos.x,v.Pos.y,v.Pos.z) <= 8 then
					Draw3DText(v.Pos.x, v.Pos.y, v.Pos.z, "~g~[E]~w~ View catalog",0.4)
				end
			end
		end
