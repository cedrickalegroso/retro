...
RegisterServerEvent('ev-inventory:openInventory')
AddEventHandler('ev-inventory:openInventory', function(inventory)
...
RegisterServerEvent('ev-inventory:closeInventory')
AddEventHandler('ev-inventory:closeInventory', function(inventory)
    if openInventory[inventory.owner] == nil then
        openInventory[inventory.owner] = {}
    end
...
RegisterServerEvent('ev-inventory:refreshInventory')
AddEventHandler('ev-inventory:refreshInventory', function(owner)
    if openInventory[owner] == nil then
        openInventory[owner] = {}
    end

    for k, v in pairs(openInventory[owner]) do
        TriggerClientEvent('ev-inventory:refreshInventory', k)
    end
end)
...
RegisterServerEvent("ev-inventory:MoveToEmpty")
    ...
    if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
        ...
        if data.originTier.name == 'shop' then
            ...
            if player.getMoney() >= data.originItem.price * data.originItem.qty then
                ...
                TriggerClientEvent('ev-inventory:refreshInventory', data.target)
                return
            end
        end
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    ...
                    TriggerEvent('ev-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, ownerPlayer.source)
...
RegisterServerEvent("ev-inventory:SwapItems")
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    ...
                    TriggerEvent('ev-inventory:notifyImpendingAddition', data.originItem, data.originItem.qty, originPlayer.source)
                    TriggerEvent('ev-inventory:notifyImpendingRemoval', data.destinationItem, data.destinationItem.qty, originPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    ...
                    data.destinationItem.block = true
                    local destinationPlayer = ESX.GetPlayerFromIdentifier(data.destinationOwner)
                    TriggerEvent('ev-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, destinationPlayer.source)
                    TriggerEvent('ev-inventory:notifyImpendingAddition', data.destinationItem, data.destinationItem.qty, destinationPlayer.source)
...
RegisterServerEvent("ev-inventory:CombineStack")
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    data.originItem.block = true
                    local originPlayer = ESX.GetPlayerFromIdentifier(data.originOwner)
                    TriggerEvent('ev-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, originPlayer.source)
...
RegisterServerEvent("ev-inventory:TopoffStack")
...
RegisterServerEvent("ev-inventory:EmptySplitStack")
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    local originPlayer = ESX.GetPlayerFromIdentifier(data.originOwner)
                    data.originItem.block = true
                    TriggerEvent('ev-inventory:notifyImpendingRemoval', data.originItem, data.moveQty, originPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    local destinationPlayer = ESX.GetPlayerFromIdentifier(data.destinationOwner)
                    data.originItem.block = true
                    TriggerEvent('ev-inventory:notifyImpendingAddition', data.originItem, data.moveQty, destinationPlayer.source)
...
RegisterServerEvent("ev-inventory:SplitStack")
...
RegisterServerEvent("ev-inventory:GiveItem")
AddEventHandler("ev-inventory:GiveItem", function(data)

    if isWeapon(data.originItem.id) then
        TriggerClientEvent('ev-inventory:removeCurrentWeapon', source)
    end

    TriggerEvent('ev-inventory:notifyImpendingRemoval', data.originItem, data.count, source)
    TriggerEvent('ev-inventory:notifyImpendingAddition', data.originItem, data.count, data.target)
...
RegisterServerEvent("ev-inventory:GiveCash")
AddEventHandler("ev-inventory:GiveCash", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("ev-inventory:CashStore")
AddEventHandler("ev-inventory:CashStore", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
    if data.item == 'cash' then
        if sourcePlayer.getMoney() >= data.count then
            sourcePlayer.removeMoney(data.count)
            ...
            invHandler.applyToInventory(data.owner, function(inventory)
                ...
                TriggerEvent('ev-inventory:refreshInventory', sourcePlayer.identifier)
                TriggerEvent('ev-inventory:refreshInventory', data.owner)
            end)
        end
    elseif data.item == 'black_money' then
        if sourcePlayer.getAccount('black_money').money >= data.count then
...
RegisterServerEvent("ev-inventory:CashTake")
AddEventHandler("ev-inventory:CashTake", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
    if data.item == 'cash' then
        local invHandler = InvType[data.destinationTier.name]
        invHandler.applyToInventory(data.owner, function(inventory)
...
ESX.RegisterServerCallback('ev-inventory:getSecondaryInventory', function(source, cb, type, identifier)
...
function saveInventories()
    ...
    RconPrint('^2[ev-inventory][SAVED] All Inventories' .. "\n^0")
end
...
function saveLoadedInventory(identifier, type, data)
    if table.length(data) > 0 then
        MySQL.Async.execute('UPDATE disc_inventory SET data = @data WHERE owner = @owner AND type = @type', {
            ...
            TriggerEvent('ev-inventory:savedInventory', identifier, type, data)
        end)
    end
end
...
function createInventory(identifier, type, data)
    MySQL.Async.execute('INSERT INTO disc_inventory (owner, type, data) VALUES (@owner, @type, @data)', {
        ...
        TriggerEvent('ev-inventory:createdInventory', identifier, type, data)
    end)
end
...
function deleteInventory(identifier, type)
    MySQL.Async.execute('DELETE FROM disc_inventory WHERE owner = @owner AND type = @type', {
        ...
        TriggerEvent('ev-inventory:deletedInventory', identifier, type)
    end)
end
...
function applyToInventory(identifier, type, f)
    ...
    if loadedInventories[type][identifier] and table.length(loadedInventories[type][identifier]) > 0 then
        TriggerEvent('ev-inventory:modifiedInventory', identifier, type, loadedInventories[type][identifier])
    else
        TriggerEvent('ev-inventory:modifiedInventory', identifier, type, nil)
    end
end
...
function loadInventory(identifier, type, cb)
    MySQL.Async.fetchAll('SELECT data FROM disc_inventory WHERE owner = @owner and type = @type', {
        ...
        cb(inventory)
        TriggerEvent('ev-inventory:loadedInventory', identifier, type, inventory)
    end)
end
