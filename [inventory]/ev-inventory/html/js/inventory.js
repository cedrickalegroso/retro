...
function closeInventory() {
    ...
    $.post("http://ev-inventory/NUIFocusOff", JSON.stringify({}));
    $('#search').val('');
    ...
    $('#use').click(function(event, ui) {
        if (dragging) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://ev-inventory/UseItem", JSON.stringify({
    ...
    $('#drop').click(function(event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                ...
                $.post("http://ev-inventory/DropItem", JSON.stringify({
                    item: itemData,
                    qty: dropCount
                }));
            } else {}
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            $.post("http://ev-inventory/MoveToEmpty", JSON.stringify({
            ...
            $.post("http://ev-inventory/EmptySplitStack", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                $.post("http://ev-inventory/SplitStack", JSON.stringify({
                    originOwner: origin.parent().data('invOwner'),
                    originTier: origin.parent().data('invTier'),
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    $.post("http://ev-inventory/SwapItems", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                    ...
                    $.post("http://ev-inventory/CombineStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    $.post("http://ev-inventory/TopoffStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
...
$('.popup-body').on('click', '.player', function() {
    ...
    if (action === "nearPlayersGive") {
        ...
        if (givingItem != null) {
            ...
            $.post("http://ev-inventory/GiveItem", JSON.stringify({
                target: target,
        ...
        $.post("http://ev-inventory/GiveCash", JSON.stringify({
            target: target,
            item: givingItem,
            count: count
        }), function(status) {
