...
window.addEventListener("message", function(event) {
    ...
    if (event.data.action == "display") {
        ...
        $("#action-bar").fadeOut(300);

        $(".ui").fadeIn(500);
    } else if (event.data.action == "updateWeight") {
        currentMaxWeight = event.data.MaxWeight.toFixed(2);
        currentWeight = event.data.Weight.toFixed(2);
    } else if (event.data.action == "updateOtherWeight") {
        currentOtherMaxWeight = event.data.MaxWeight.toFixed(2);
        currentOtherWeight = event.data.Weight.toFixed(2);
    } else if (event.data.action == "setStreet") {
        street = event.data.street;
    } else if (event.data.action == "hide") {
        if (event.data.type == 'secondary') {
            $("#inventoryTwo").fadeOut(500);
        } else {
            $("#dialog").dialog("close");
            $(".ui").fadeOut(500);
        }
...
function closeInventory() {
    ...
    InventoryLog('Closing Inventory.');
    EndDragging();
    $('.near-players-wrapper').fadeOut();
    $.post("http://retro-inventory/NUIFocusOff", JSON.stringify({}));
    $('#search').val('');
...
function setupSecondarySlots(owner) {
    ...
    for (i = 1; i <= (secondTier.slots); i++) {
        ...
        if (owner.search('x') == 0 && owner.search('y') == 0 && owner.search('z') == 0) {
            $('#inventoryTwo').find('.slot-template').addClass('drop');
...
$(document).ready(function() {
    ...
    $('#use').click(function(event, ui) {
        if (dragging) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://retro-inventory/UseItem", JSON.stringify({
    ...
    $('#give').click(function(event, ui) {
        if (draggingItem != null && dragging) {
            ...
            if (itemData.canRemove) {
                $.post("http://retro-inventory/GetNearPlayers", JSON.stringify({
                    originItem: itemData,
                    action: 'give'
                }));
    ...
    $("#seize").mouseenter(function() {
        ...
        $.post("http://retro-inventory/SeizeCash", JSON.stringify({
    ...
    $("#steal").mouseenter(function() {
        ...
        $.post("http://retro-inventory/StealCash", JSON.stringify({
            target: destinationOwner
        }));
    });
    ...
    $('#drop').click(function(event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                InventoryLog('Dropping ' + dropCount + ' ' + itemData.label + ' On The Ground');
                $.post("http://retro-inventory/DropItem", JSON.stringify({
                    item: itemData,
                    qty: dropCount
                }));
            } else {}
        ...
        if (itemData !== undefined) {
            ...
            if (!itemData.unique) {
                if (itemData.stackable) {
                    $('.Information').find('.info-uniqueness').html("Not Unique - Stack Max (" + itemData.max + ")");
                } else {
                    $('.Information').find('.info-uniqueness').html("Not Unique - Not Stackable");
                }
            } else {
                $('.Information').find('.info-uniqueness').html("Unique (" + itemData.max + ")");
            }

            if (itemData.description !== undefined) {
                $('.Information').find('.info-desc').html('Description: ' + itemData.description);
            } else {
...
$('.popup-body').on('click', '.cashstore', function() {
    $.post("http://retro-inventory/CashStore", JSON.stringify({
        action: 'cashstore',
        item: $(this).data("id"),
...
$('.popup-body').on('click', '.cashtake', function() {
    $.post("http://retro-inventory/CashTake", JSON.stringify({
        action: 'cashtake',
        item: $(this).data("id"),
        count: parseInt($("#count").val()),
        owner: destinationOwner,
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            $.post("http://retro-inventory/MoveToEmpty", JSON.stringify({
            ...
            $.post("http://retro-inventory/EmptySplitStack", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                $.post("http://retro-inventory/SplitStack", JSON.stringify({
                    originOwner: origin.parent().data('invOwner'),
                    originTier: origin.parent().data('invTier'),
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    $.post("http://retro-inventory/SwapItems", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                    ...
                    $.post("http://retro-inventory/CombineStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    $.post("http://retro-inventory/TopoffStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
...
$('.popup-body').on('click', '.player', function() {
    ...
    if (action === "nearPlayersGive") {
        ...
        if (givingItem != null) {
            ...
            $.post("http://retro-inventory/GiveItem", JSON.stringify({
                target: target,
        ...
        $.post("http://retro-inventory/GiveCash", JSON.stringify({
            target: target,
            item: givingItem,
            count: count
        }), function(status) {
    ...
    if ($('#action-bar').is(':visible')) {
        ...
        for (let i = 0; i < 4; i++) {
            if (items[i] != null) {
                ...
                $('#action-bar').append(`<div class="slot slot-${i}" data-empty="true"><div class="item"><div class="item-count"></div><div class="item-name">Empy Slot</div><div class="item-keybind">${i + 1}</div></div></div>`);
