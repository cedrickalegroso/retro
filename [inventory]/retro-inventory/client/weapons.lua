...
RegisterNetEvent('retro-inventory:useWeapon')
AddEventHandler('retro-inventory:useWeapon', function(weapon)
...
RegisterNetEvent('retro-inventory:removeCurrentWeapon')
AddEventHandler('retro-inventory:removeCurrentWeapon', function()
...
function RemoveWeapon(weapon)
    ...
    TriggerServerEvent('retro-inventory:updateAmmoCount', hash, ammoCount)
...
function GiveWeapon(weapon)
    ...
    local playerPed = GetPlayerPed(-1)
    local hash = GetHashKey(weapon)
    ESX.TriggerServerCallback('retro-inventory:getAmmoCount', function(ammoCount)
            ...
            for k, v in pairs(Config.Throwables) do
                if k == currentWeapon then
                    ...
                    ESX.TriggerServerCallback('disc-base:takePlayerItem', function(removed)
                        if removed then
                            TriggerEvent('retro-inventory:removeCurrentWeapon')
                        end
                    end, currentWeapon, 1)
                end
            end
