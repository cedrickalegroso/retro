    ...
    MySQL.Async.fetchAll('SELECT * FROM items WHERE LCASE(name) LIKE \'%weapon_%\'', {}, function(results)
        for k, v in pairs(results) do
            ESX.RegisterUsableItem(v.name, function(source)
                TriggerClientEvent('retro-inventory:useWeapon', source, v.name)
            end)
        end
    end)
end)

RegisterServerEvent('retro-inventory:updateAmmoCount')
AddEventHandler('retro-inventory:updateAmmoCount', function(hash, count)
...
ESX.RegisterServerCallback('retro-inventory:getAmmoCount', function(source, cb, hash)
    local player = ESX.GetPlayerFromId(source)
