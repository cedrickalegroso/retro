ESX.RegisterServerCallback('retro-inventory:UseItemFromSlot', function(source, cb, slot)
...
ESX.RegisterServerCallback('retro-inventory:GetItemsInSlotsDisplay', function(source, cb)
    local player = ESX.GetPlayerFromId(source)
    InvType['player'].getInventory(player.identifier, function(inventory)
        local slotItems = {}
        for i = 1, 5, 1 do
...
ESX.RegisterServerCallback('retro-inventory:getPlayerIdFromSteam', function(source, cb, steam)
    local player = ESX.GetPlayerFromIdentifier(steam)
    local id = player.source or 0
    cb(id)
end)