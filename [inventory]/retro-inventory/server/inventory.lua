...
RegisterServerEvent('retro-inventory:openInventory')
AddEventHandler('retro-inventory:openInventory', function(inventory)
...
RegisterServerEvent('retro-inventory:closeInventory')
AddEventHandler('retro-inventory:closeInventory', function(inventory)
    if openInventory[inventory.owner] == nil then
        openInventory[inventory.owner] = {}
    end
...
RegisterServerEvent('retro-inventory:refreshInventory')
AddEventHandler('retro-inventory:refreshInventory', function(owner)
    if openInventory[owner] == nil then
        openInventory[owner] = {}
    end

    for k, v in pairs(openInventory[owner]) do
        TriggerClientEvent('retro-inventory:refreshInventory', k)
    end
end)
...
RegisterServerEvent("retro-inventory:MoveToEmpty")
AddEventHandler("retro-inventory:MoveToEmpty", function(data)
    ...
    if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
        ...
        originInvHandler.applyToInventory(data.originOwner, function(inventory)
            ...
            TriggerEvent('retro-inventory:refreshInventory', data.originOwner)
        end)
    else
        local originInvHandler = InvType[data.originTier.name]
        ...
        if data.originTier.name == 'shop' then
            ...
            if player.getMoney() >= data.originItem.price * data.originItem.qty then
                ...
                TriggerClientEvent('retro-inventory:refreshInventory', data.target)
                return
            end
        end
        ...
        if data.destinationTier.name == 'shop' then
            TriggerEvent('retro-inventory:refreshInventory', data.originOwner)
            TriggerEvent('retro-inventory:refreshInventory', data.destinationOwner)
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    ...
                    TriggerEvent('retro-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, ownerPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    ...
                    TriggerEvent('retro-inventory:notifyImpendingAddition', data.originItem, data.originItem.qty, destinationPlayer.source)
...
RegisterServerEvent("retro-inventory:SwapItems")
    ...
    if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                originInventory[tostring(data.originSlot)] = destinationInventory[tostring(data.destination)]
                ...
                if data.originTier.name == 'player' then
                    ...
                    TriggerEvent('retro-inventory:notifyImpendingAddition', data.originItem, data.originItem.qty, originPlayer.source)
                    TriggerEvent('retro-inventory:notifyImpendingRemoval', data.destinationItem, data.destinationItem.qty, originPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    ...
                    data.destinationItem.block = true
                    local destinationPlayer = ESX.GetPlayerFromIdentifier(data.destinationOwner)
                    TriggerEvent('retro-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, destinationPlayer.source)
                    TriggerEvent('retro-inventory:notifyImpendingAddition', data.destinationItem, data.destinationItem.qty, destinationPlayer.source)
...
RegisterServerEvent("retro-inventory:CombineStack")
AddEventHandler("retro-inventory:CombineStack", function(data)
    ...
    if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
        local originInvHandler = InvType[data.originTier.name]
        originInvHandler.applyToInventory(data.originOwner, function(inventory)
            inventory[tostring(data.origin)] = nil
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    data.originItem.block = true
                    local originPlayer = ESX.GetPlayerFromIdentifier(data.originOwner)
                    TriggerEvent('retro-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, originPlayer.source)
...
RegisterServerEvent("retro-inventory:TopoffStack")
...
RegisterServerEvent("retro-inventory:EmptySplitStack")
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    local originPlayer = ESX.GetPlayerFromIdentifier(data.originOwner)
                    data.originItem.block = true
                    TriggerEvent('retro-inventory:notifyImpendingRemoval', data.originItem, data.moveQty, originPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    local destinationPlayer = ESX.GetPlayerFromIdentifier(data.destinationOwner)
                    data.originItem.block = true
                    TriggerEvent('retro-inventory:notifyImpendingAddition', data.originItem, data.moveQty, destinationPlayer.source)
...
RegisterServerEvent("retro-inventory:SplitStack")
...
RegisterServerEvent("retro-inventory:GiveItem")
AddEventHandler("retro-inventory:GiveItem", function(data)

    if isWeapon(data.originItem.id) then
        TriggerClientEvent('retro-inventory:removeCurrentWeapon', source)
    end

    TriggerEvent('retro-inventory:notifyImpendingRemoval', data.originItem, data.count, source)
    TriggerEvent('retro-inventory:notifyImpendingAddition', data.originItem, data.count, data.target)
...
RegisterServerEvent("retro-inventory:GiveCash")
AddEventHandler("retro-inventory:GiveCash", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("retro-inventory:CashStore")
AddEventHandler("retro-inventory:CashStore", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
    if data.item == 'cash' then
        if sourcePlayer.getMoney() >= data.count then
            sourcePlayer.removeMoney(data.count)
            ...
            invHandler.applyToInventory(data.owner, function(inventory)
                ...
                TriggerEvent('retro-inventory:refreshInventory', sourcePlayer.identifier)
                TriggerEvent('retro-inventory:refreshInventory', data.owner)
            end)
        end
    elseif data.item == 'black_money' then
        if sourcePlayer.getAccount('black_money').money >= data.count then
...
RegisterServerEvent("retro-inventory:CashTake")
AddEventHandler("retro-inventory:CashTake", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
    if data.item == 'cash' then
        local invHandler = InvType[data.destinationTier.name]
        invHandler.applyToInventory(data.owner, function(inventory)
...
ESX.RegisterServerCallback('retro-inventory:getSecondaryInventory', function(source, cb, type, identifier)
...
function saveInventories()
    ...
    RconPrint('^2[retro-inventory][SAVED] All Inventories' .. "\n^0")
end
...
function saveLoadedInventory(identifier, type, data)
    if table.length(data) > 0 then
        MySQL.Async.execute('UPDATE disc_inventory SET data = @data WHERE owner = @owner AND type = @type', {
            ...
            TriggerEvent('retro-inventory:savedInventory', identifier, type, data)
        end)
    end
end
...
function createInventory(identifier, type, data)
    MySQL.Async.execute('INSERT INTO disc_inventory (owner, type, data) VALUES (@owner, @type, @data)', {
        ...
        TriggerEvent('retro-inventory:createdInventory', identifier, type, data)
    end)
end
...
function deleteInventory(identifier, type)
    MySQL.Async.execute('DELETE FROM disc_inventory WHERE owner = @owner AND type = @type', {
        ...
        TriggerEvent('retro-inventory:deletedInventory', identifier, type)
    end)
end
...
function applyToInventory(identifier, type, f)
    ...
    if loadedInventories[type][identifier] and table.length(loadedInventories[type][identifier]) > 0 then
        TriggerEvent('retro-inventory:modifiedInventory', identifier, type, loadedInventories[type][identifier])
    else
        TriggerEvent('retro-inventory:modifiedInventory', identifier, type, nil)
    end
end
...
function loadInventory(identifier, type, cb)
    MySQL.Async.fetchAll('SELECT data FROM disc_inventory WHERE owner = @owner and type = @type', {
        ...
        cb(inventory)
        TriggerEvent('retro-inventory:loadedInventory', identifier, type, inventory)
    end)
end
