...
RegisterServerEvent('retro-inventory:RegisterInventory')
AddEventHandler('retro-inventory:RegisterInventory', function(inventory)
...
ESX.RegisterServerCallback('retro-inventory:doesInvTypeExists', function(source, cb, type)
    cb(InvType[type] ~= nil)
end)
...
RegisterServerEvent('retro-inventory:deleteNilInventory')
AddEventHandler('retro-inventory:deleteNilInventory', function()
    local owner = ESX.GetPlayerFromId(source).identifier
    MySQL.Async.fetchAll('DELETE FROM disc_inventory WHERE data = @data AND owner = @owner', { ['@data'] = "null", ['@owner'] = owner })
end)