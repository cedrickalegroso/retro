local hospitalCheckin = { x =0, y = 0, z =0}
...
AddEventHandler('mythic_hospital:client:SendToBed', function(id, data)
    ...
    bedObject = GetClosestObjectOfType( -454.46453857422,-282.16537475586,34.913360595703, 1.0, data.model, false, false, false)
    FreezeEntityPosition(bedObject, true)
    ...
    while true do
        ...
        if distance < 10 then
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                if distance < 3 then
                    DrawText3Ds(hospitalCheckin.x, hospitalCheckin.y, hospitalCheckin.z + 0.5, 'Press ~r~[E]~s~ To Check in')
                        ...
                        if (GetEntityHealth(PlayerPedId()) < 200) then
                            ...
                            exports['progressbar']:Progress({
                                ...
                                if not status then
                                    ...
                                    TriggerServerEvent('mythic_hospital:pay', 250)
                                    TriggerEvent('esx:showNotification', '~w~You Were Billed For ~r~$250 ~w~For Medical Services & Expenses')
                                    TriggerEvent('mythic_hospital:client:RemoveBleed')
                                    TriggerEvent('mythic_hospital:client:ResetLimbs')
                                end
                            end)
