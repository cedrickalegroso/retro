...
AddEventHandler('police:client:UpdateBlips', function(players)
    ...
    if (ESX.PlayerData.job ~= nil and players[GetPlayerServerId(PlayerId())]) and (ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance') then
        if players ~= nil then
...
AddEventHandler('impound', function()
    ...
    if not IsPedInAnyVehicle(ped, false) and DoesEntityExist(vehicle) and ( Vdist2(GetEntityCoords(vehicle), coords) < 5 ) then
        ...
        if ( Vdist2(GetEntityCoords(vehicle), coords) < 5 ) then
            TriggerServerEvent("police:server:impound", GetVehicleNumberPlateText(vehicle))
