...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	Progressbar("clear_blooddrops", "Taking blood..", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			TriggerEvent('notification', "Blood taken away :)")
		end
    end, function() -- Cancel
        TriggerEvent('notification', "Blood not removed!", 2)
    end)
end)
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(GetPlayerPed(-1))
	local casingList = {}
	Progressbar("clear_casings", "Remove the ball sleeves..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			TriggerEvent('notification', "Bullet sleeves removed :)")
		end
    end, function() -- Cancel
        TriggerEvent('notification', "Ball sleeves not removed!", 2)
    end)
end)
