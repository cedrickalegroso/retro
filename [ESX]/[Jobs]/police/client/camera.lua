...
Citizen.CreateThread(function()
    while true do
        ...
        for k,v in pairs(Config.Shops) do
            for i=1, #v.Cams, 1 do
                local dis = #(coords - v.Cams[i])
                if dis < 1.2 then
                    YMDrawText3D(v.Cams[i][1], v.Cams[i][2], v.Cams[i][3], 'Press ~r~[E]~s~ To View Cameras')
                    local job = exports["isPed"]:isPed("myjob")
                    if dis < 3 and IsControlJustPressed(1, 38) then
                        if (job == "police") then
                            ESX.Streaming.RequestAnimDict("mp_fbi_heist", function()
                                TaskPlayAnim(PlayerPedId(), "mp_fbi_heist", "loop", 8.0, -8.0, -1, 0, 0, false, false, false)
                                exports['taskbar']:taskBar(7000, "Watching cameras")
                                TriggerEvent('YM:Cams')
                            end)
                        elseif (job ~= "police") then
                            TriggerEvent("notification",'Your Not A Officer',2)
                        end
                    end
                end
            end
...
AddEventHandler('police:client:ActiveCamera', function(cameraId)
    ...
    elseif cameraId == 0 then
        ...
        TriggerEvent('notification', "Camera bestaat niet..", 2)
    end
end)
