...
RegisterNetEvent('YM:policerob')
AddEventHandler('YM:policerob', function()
...
AddEventHandler('police:client:PutPlayerInVehicle', function()
    ...
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if (not isHandcuffed and not isEscorted) then
            TriggerServerEvent('police:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "set " .. GetPlayerName(player) .. "(" .. playerId .. ") in vehicle")
...
AddEventHandler('police:client:SetPlayerOutVehicle', function()
    ...
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent('police:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "set " .. GetPlayerName(player) .. "(" .. playerId .. ") out of vehicle")
...
AddEventHandler('police:client:EscortPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent('police:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Escort " .. GetPlayerName(player) .. "(" .. playerId .. ")")
...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if not IsPedInAnyVehicle(GetPlayerPed(player)) then
            if not isHandcuffed and not isEscorted then
                TriggerServerEvent('police:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Is robbing " .. GetPlayerName(player) .. "(" .. playerId .. ")")
...
RegisterNetEvent('police:client:fineMenu')
AddEventHandler('police:client:fineMenu', function()
...
AddEventHandler('police:client:CuffPlayerSoft', function()
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim ( GetPlayerPed(player), "random@mugging3", "handsup_standing_base", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "random@arrests@busted", "idle_a", 3) or ESX.PlayerData.job.name == 'police') then
                TriggerServerEvent('police:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Cuff (Soft) " .. GetPlayerName(player) .. "(" .. playerId .. ")")
...
AddEventHandler('police:client:CuffPlayer', function()
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim ( GetPlayerPed(player), "random@mugging3", "handsup_standing_base", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "random@arrests@busted", "idle_a", 3) or ESX.PlayerData.job.name == 'police') then
                TriggerServerEvent('police:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Cuff (Hard) " .. GetPlayerName(player) .. "(" .. playerId .. ")")
...
AddEventHandler('police:client:UnCuffPlayer', function()
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim(GetPlayerPed(player), "mp_arresting", "idle", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "mp_arrest_paired", "crook_p2_back_right", 3) or ESX.PlayerData.job.name == 'police') then
                TriggerServerEvent('police:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Uncuffed " .. GetPlayerName(player) .. '(' .. playerId .. ')') 
...
AddEventHandler('police:client:GetUnCuffed', function(playerId)
    isHandcuffed = false
...
function IsTargetDead(playerId)
    local retval = false
    ESX.TriggerServerCallback('police:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Citizen.Wait(100)
    return retval
...
RegisterCommand('remask', function()
    TriggerEvent('police:remmask')
end)

RegisterNetEvent('police:remmaskAccepted')
AddEventHandler('police:remmaskAccepted', function()
	TriggerEvent("police:facewearAdjust", 1, true)
	TriggerEvent("police:facewearAdjust", 3, true)
	TriggerEvent("police:facewearAdjust", 4, true)
	TriggerEvent("police:facewearAdjust", 2, true)
end)

RegisterNetEvent('police:remmask')
AddEventHandler('police:remmask', function()
...
RegisterNetEvent("police:facewearAdjust")
AddEventHandler("police:facewearAdjust",function(faceType,remove)
	local AnimSet = "none"
