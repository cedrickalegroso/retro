...
local MechanicLocation = vector3(-211.55, -1324.55, 30.90)

local bennyHeading = 274.34
...
local function saveVehicle()
    ...
    for k,v in pairs(vehProps) do
        if v ~= LastVehicleData[k] then
            msg = msg .. k .. ', '
        end
    end
    print(msg)
    TriggerServerEvent('mechanic:refreshVehicle', vehProps)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("mechanic:attemptPurchase", type, upgradeLevel)
...
function enterLocation(locationsPos)
    ...
    LastVehicleData = ESX.Game.GetVehicleProperties(plyVeh)
...
--#[Event Handlers]#--
RegisterNetEvent("mechanic:purchaseSuccessful")
AddEventHandler("mechanic:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("mechanic:purchaseFailed")
AddEventHandler("mechanic:purchaseFailed", function()
