...
local MechanicLocation = vector3(-211.08528137207, -1323.9572753906, 30.89040184021)
        ...
        if IsPedInAnyVehicle(plyPed, false) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
            ...
            if nearDefault then
                 ...
                 if not isPlyInBennys then
                    ...
                    Draw3DText(MechanicLocation.x, MechanicLocation.y, MechanicLocation.z + 0.5, "[Press ~p~E~w~ - Enter Mechanic Menu]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
    ...
    while true do
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
            for k,v in pairs(Config.Locations['stash']) do
                ...
                if d < 2 then
                    DrawText3Ds(v.x, v.y, v.z, '[E] - Stash')
                    if IsControlJustPressed(0,38) then
            ...
            if ESX.PlayerData.job.grade_name == 'boss' then
                for k,v in pairs(Config.Locations['boss']) do
                    local d = Vdist2(GetEntityCoords(PlayerPedId()), v.x, v.y, v.z)
                    if d < 2 then
                        DrawText3Ds(v.x, v.y, v.z, '[E] - Boss Menu')
                        if IsControlJustPressed(0,38) then
...
AddEventHandler('mechanic:bill', function()
    ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
        ...
        if amount == nil or amount < 0 then
            TriggerEvent('notification', 'Amount invalid.', 2)
        else
...
AddEventHandler('mechanic:onHijack', function()
    ...
    if IsPedSittingInAnyVehicle(playerPed) then
        TriggerEvent('notification', 'Inside vehicle.', 2)
        return
    elseif DoesEntityExist(vehicle) then
        TaskStartScenarioInPlace(playerPed, 'WORLD_HUMAN_WELDING', 0, true)
        exports["taskbar"]:taskBar(6000,"Hijack vehicle")
...
AddEventHandler('mechanic:repair', function()
    ...
    elseif DoesEntityExist(vehicle) then
        ...
        exports["taskbar"]:taskBar(10000,"Repair vehicle")
        SetVehicleFixed(vehicle)
        SetVehicleDeformationFixed(vehicle)
        SetVehicleUndriveable(vehicle, false)
        SetVehicleEngineOn(vehicle, true, true)
        ClearPedTasksImmediately(playerPed)
        TriggerEvent('notification', 'Vehicle repaired.', 1)
    else
        TriggerEvent('notification', 'No vehicle nearby.', 2)
    end
end)
